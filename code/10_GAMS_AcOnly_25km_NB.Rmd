...
require(mgcv)
nbS999 <- gam(pa ~ s(bath_m, k=3) + s(dist, k=3)  + s(d2smt, k=3) +  s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(ssh, k=3) + s(sshsd, k=3) + s(eke, k=3) + s(wavepow, k=3) + offset(log.effort), data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbS999)
AIC(nbS999)        
```
```{r echo=FALSE}
...
```{r}
nbS999LL <- gam(pa ~ s(Longitude, Latitude) + s(bath_m, k=3) + s(dist, k=3) + s(d2smt, k=3) +  s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(ssh, k=3) + s(sshsd, k=3) + s(eke, k=3) + s(wavepow, k=3) + offset(log.effort), data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbS999LL)
AIC(nbS999LL)
```
```{r echo=FALSE}
...
### Reduced Models  
   ...
   * Keep:   
      ...
      + EKE  
```{r echo=FALSE,eval=FALSE}
nbS999LLb <- gam(pa ~ s(Longitude, Latitude) + s(ssh, k=3) + s(sshsd, k=3) + s(eke, k=3) + offset(log.effort), data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbS999LLb)
```

```{r echo=FALSE,eval=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(nbS999LLb, residuals = FALSE, pch = 20, cex = 0.001,
...
nbS999LLb2 <- gam(pa ~ s(Longitude, Latitude) + s(SSH, k=3)  + s(SSHsd, k=3) + s(EKE, k=3) + offset(log.effort), data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbS999LLb2)
```
```{r}
# model diagnostics
