...
### 2022: Redo Click Codes -> saved as 'v2'
```{r}
...
saveRDS(PmCombo, here::here(paste0('data/Pm_Combined_2022_v2.rda') ))
...
modForageTW  <- gam(pa ~  s(Longitude, Latitude, k=12) + s(Depth, k=3) + s(DistShore, k=3)  + s(DistSeamt, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.EKE, k=3) + s(log.WavePower, k=3) + s(year, k=3) + s(array, k=3) + offset(log.effort), data = trainHunt, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modForageTW) 

#train80a - USED in paper
modForageTW2  <- gam(pa ~  s(Longitude, Latitude, k=12) +  s(Temp584m, k=3) + s(log.Chla, k=3) + offset(log.effort) , data = trainHunt, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modForageTW2) 
concurvity(modForageTW2, full=FALSE)
...
## FORAGE-NO s(x,y)
```{r}
modForageTWb  <- gam(pa ~ s(Depth, k=3) + s(DistShore, k=3)  + s(DistSeamt, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.EKE, k=3) + s(log.WavePower, k=3) + offset(log.effort) +  s(year, k=3) , data = trainHunt, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modForageTWb) 

modForageTWc  <- gam(pa ~  s(Temp584m, k=3) + s(SSH, k=3) + offset(log.effort) , data = trainHunt, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modForageTWc) 

modForageNBb  <- gam(pa ~ s(Depth, k=3) + s(DistShore, k=3)  + s(DistSeamt, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.EKE, k=3) + s(log.WavePower, k=3) + offset(log.effort) +  s(year, k=3) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForageNBb) 

modForageNBc  <- gam(pa ~ s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + offset(log.effort) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForageNBc)
...
** Save using the R Export and choose 1200 x 800 to get smaller black dots.
...
png(file = "C:/Users/yvers/OneDrive/Documents/Publications/2022_FiRS_SDMs/figures/Foraging_Smoother2Dplot_16kmB.png", width = 32, height = 16, res = 300, units = "cm") # 8x6"

# pdf(file = "C:/Users/yvers/OneDrive/Documents/Publications/2022_FiRS_SDMs/figures/Foraging_Smoother2Dplot_16kmB.pdf", width = 7, height = 5)
...
points(whales$Longitude, whales$Latitude, pch = 16, col='deepskyblue', #mediumslateblue #000DFF
       lwd=1.25, cex=1.2)
points(hawaiiMap$Longitude2, hawaiiMap$Latitude, pch=20, cex = 0.001, col='white')
...
summary(modNonForage)
concurvity(modNonForage, full=FALSE)
...
## NONFORAGE-NO s(x,y)
```{r}
modNonForageTWb  <- gam(pa ~ s(Depth, k=3) + s(DistShore, k=3)  + s(DistSeamt, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.EKE, k=3) + s(log.WavePower, k=3) + offset(log.effort) +  s(year, k=3) , data = trainNonFor, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modNonForageTWb) 

modNonForageTWc  <- gam(pa ~ s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + offset(log.effort) , data = trainNonFor, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modNonForageTWc) 
...
#for 2D smoother plot
points(whales$Longitude, whales$Latitude, pch = 16, col='deepskyblue', #mediumslateblue
       lwd=1.25, cex=1.2)
points(hawaiiMap$Longitude2, hawaiiMap$Latitude, pch=20, cex = 0.001, col = 'white')
...
points(whales$Longitude, whales$Latitude, pch = 16, col='#000DFF', #mediumslateblue
       lwd=1.25, cex=1.2)
points(hawaiiMap$Longitude2, hawaiiMap$Latitude, pch=20, cex = 0.1)
