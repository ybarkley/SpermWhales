---
title: "ALL SURVEYS AT ONCE - Environmental Data"
...
# sw=readRDS(here::here(paste0('Documents\\PHD\\Chp 3\\data\\centroids\\', gridsize, ' km'), paste0('centroids', surveynum, '_', gridsize, 'km_loc.rda'))) #using NOAA laptop
# sw <- readRDS(here::here('output', paste0('centroids', surveynum, '_', gridsize, 'km.rda')))
# sw <- readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\ProjectSpermie_Desktop\\centroids\\5 km\\centroids1604_5km_loc.rda')
sw <- readRDS( paste0('output/centroids/', gridsize, ' km-', loctype, '/', 'centroids_AllSurveys_', gridsize, 'km_', loctype2, '.rda' ) )
...
#new
# minUTC <- min(sw$UTC)
# minUTC <- str_sub(minUTC, 1,10)
# minUTC <- ymd_hms(paste0(minUTC, ' 00:00:00'))
# 
# maxUTC <- max(sw$UTC)
# maxYr <- str_sub(maxUTC, 1,4)
# maxMo <- as.numeric(str_sub(maxUTC, 6,7))+1
# maxDay <- str_sub(maxUTC, 9,10)
# maxUTC <- ymd_hms(paste0(maxYr,'-',maxMo,'-',maxDay, ' 00:00:00'))
# maxUTC <- ymd_hms(paste0(2011,'-',01,'-',maxDay, ' 00:00:00'))  #Use this if max dates in Dec
# minUTC <- ymd_hms(paste0(maxYr,'-',08,'-',01, ' 00:00:00'))  #Use this to adjust min date
...
#5. Bathymetry: https://topex.ucsd.edu/WWW_html/srtm15_plus.html, 180 to -180
# bathy_tot <- readRDS(here::here('data', 'bathy_tot.rda')) #laptop 
# bathy_tot <- readRDS('data/bathy_tot.rda') #desktop
# bathy_tot <- readRDS(here::here('Documents\\PHD\\Chp 3\\data\\bathy_tot.rda')) #NOAA
bathy_tot <- readRDS('E:\\BACKUP-PhD\\Chapter 3-Sperm SDM\\code_data\\SpermWhales\\data\\bathy_tot.rda')

#8. Distance to Land: http://www.soest.hawaii.edu/pwessel/gshhg/, "dist_to_GSHHG_v2.3.7_1m.nc" 
# Should all be negative. Otherwise the value is from inland.
# Also uses nc file located at the VERY BOTTOM of the webpage, so far at the bottom that it's super easy to miss.
# dist_slice <-readRDS(here::here('data', 'dist2land.rda'))  #pers laptop
# dist_slice <-readRDS('data/dist2land.rda')  #desktop
# dist_slice <-readRDS(here::here('Documents\\PHD\\Chp 3\\data\\dist2land.rda'))  #NOAA
dist_slice <- readRDS('E:\\BACKUP-PhD\\Chapter 3-Sperm SDM\\code_data\\SpermWhales\\data\\dist2land.rda')


#3. Temperature at 600 m: http://apdrc.soest.hawaii.edu/erddap/griddap/hawaii_soest_d346_28ac_fccf.html,  0 to 360
##choose a depth for dep (100, 500, 600)
# As of June 2020, only using temp at 600m since 100, 500, and 600 are correlated.

dep = 600
godas = paste0('godas', dep)

#4. Wave power: http://apdrc.soest.hawaii.edu/erddap/griddap/hawaii_soest_98bb_253a_eb1c.html,  0 to 360


#6/7. Slope and Aspect, same as bathymetry dataset, 180 to -180
# slope_aspdf2 <-readRDS(here::here('data', 'slope_aspHIdf.rda')) #laptop
# slope_aspdf2 <-readRDS('data\\slope_aspHIdf.rda') #desktop
# slope_aspdf2 <- readRDS(here::here('Documents\\PHD\\Chp 3\\data\\slope_aspHIdf.rda')) #NOAA
slope_aspdf2 <- readRDS('E:\\BACKUP-PhD\\Chapter 3-Sperm SDM\\code_data\\SpermWhales\\data\\slope_aspHIdf.rda')

slope_aspdf2 <- as.data.frame(slope_aspdf2)
```

## Run for each survey to truncate dates
```{r}
#9. SSH: https://resources.marine.copernicus.eu/?option=com_csw&task=results?option=com_csw&view=details&product_id=GLOBAL_REANALYSIS_PHY_001_030,  0 to 360
# sshtidya <-readRDS(here::here('data', 'ssh_tidy.rda')) #laptop
sshtidya <-readRDS('data\\ssh_tidy.rda') #desktop
# sshtidya <- readRDS(here::here('Documents\\PHD\\Chp 3\\data\\ssh_tidy.rda')) #NOAA
sshtidya <- readRDS('E:\\BACKUP-PhD\\Chapter 3-Sperm SDM\\code_data\\SpermWhales\\data\\ssh_tidy.rda')

# sshtidya= dplyr::filter(sshtidyaTMP, date >= ymd_hms('2016-06-01 00:00:00') & date < ymd_hms('2016-08-30 00:00:00'))
# sshtidya = dplyr::filter(sshtidyaTMP, date >= minUTC & date < maxUTC)
# min(sshtidya$date)
# max(sshtidya$date)
#10. SSHsd: same as ssh,  0-360
# sshZtidya<-readRDS(here::here('data', 'sshsd_tidy.rda')) #laptop
sshZtidya <-readRDS('data\\sshsd_tidy.rda') #desktop
# sshZtidya <- readRDS(here::here('Documents\\PHD\\Chp 3\\data\\sshsd_tidy.rda')) #NOAA
sshZtidya <- readRDS('E:\\BACKUP-PhD\\Chapter 3-Sperm SDM\\code_data\\SpermWhales\\data\\sshsd_tidy.rda')

# sshZtidya= filter(sshZtidyaTMP, date >= minUTC & date < maxUTC)
#11. EKE: same as ssh, using U and V data, 0-360
# ekeZtidya<-readRDS(here::here('data','eke_tidy.rda')) #laptop
ekeZtidya <-readRDS('data\\eke_tidy.rda') #desktop
# ekeZtidya<-readRDS(here::here('Documents\\PHD\\Chp 3\\data\\eke_tidy.rda')) #NOAA
ekeZtidya <- readRDS('E:\\BACKUP-PhD\\Chapter 3-Sperm SDM\\code_data\\SpermWhales\\data\\eke_tidy.rda')

# ekeZtidya= filter(ekeZtidyaTMP, date >= minUTC & date < maxUTC)

# write.csv(sshtidya, 'C:\\Users\\yvers\\Documents\\CHP 3\\SpermWhales\\data\\For Zack-data/SSH.csv' )
# write.csv(sshZtidya, 'C:\\Users\\yvers\\Documents\\CHP 3\\SpermWhales\\data\\For Zack-data/SSHsd.csv' )
# write.csv(ekeZtidya, 'C:\\Users\\yvers\\Documents\\CHP 3\\SpermWhales\\data\\For Zack-data/EKE.csv' )


```


#RUN Env Data Extraction

##1. Start with Bathymetry
Extract bathymetry, then make histograms to examine it across the survey data.
Goal is to determine that 1000m is a reasonable truncation distance to bound the data.
```{r}
...
for (i in 1:nrow(sw)){
  ...
  btmp$survey <- sw$survey[i] #added survey number too
  
  env5_bath <- rbind(env5_bath, btmp)
}
names(env5_bath) = c("bath_m", "matched_lon", "matched_lat", "true_lon", "true_lat", "effCells", "survey")
# saveBATH <- env5_bathAll

(min_bath=proc.time()-ptm )
min_bath[3]/60 

#### SAVE All bathy data ####
saveRDS(env5_bath, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2,'_bathyAll.rda') ))

# env5_bath <- readRDS( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2,'_bathyAll.rda') ) 



##2. Evaluate the bathy data.
# Truncate at 700 m if data support it.


#Evaluate Bathymetry truncation distance by looking at bathy in the data
swdat1=sw[,c(1:3, 5:8)]
swdat2=env5_bath[,c(1,6)]

swdat3=distinct(merge(swdat1, swdat2, by='effCells', all.x=TRUE))  #merge without getting duplicates
swdat3_pa1 = filter(swdat3, pa>0)
swdat3_pa0 = filter(swdat3, pa==0)

histogram(swdat3$bath_m, main='All Bathymetry')
histogram(swdat3_pa1$bath_m, main = "Bathymetry of Whale")
histogram(swdat3_pa0$bath_m, main = "Bathymetry of Absences")

env5_bathAll <- env5_bath
env5_bath <- env5_bathAll

tmp <- cbind(sw, env5_bathAll) #combine dfs
tmp2 <- subset(tmp, bath_m <= -700) #subset bathymetry
tmp3 <- tmp2[, -c(9:15)] #results in 'sw' variable with subsetted bathymetry
tmp4 <- tmp2[, -c(1:8)]
sw = tmp3
env5_bath = tmp4
names(env5_bath) = c("bath_m", "matched_lon", "matched_lat", "true_lon", "true_lat", "effCells", "survey")
#### SAVE subset of bathy data ####
# saveRDS(env5_bath, here::here(paste0('Documents\\PHD\\Chp 3\\',surveynum, '_', gridsize, 'km_', loctype2, '_bathy.rda'))) #VM
saveRDS(env5_bath, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_bathy.rda') )) #Desktop

saveRDS(sw, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2,'_sw.rda') ))


##3. Extract Remaining Data

#### 8. Distance to Land ####
## Use 0-360 for lon, also for temp 600 and wave power
lon=sw$lon
lat=sw$lat
dates=sw$UTC
...
for (i in 1:nrow(sw)){
  ...
  dtmp$survey <- sw$survey[i] #added survey number too
  
  env8_d2land <- rbind(env8_d2land, dtmp)
}


(min_d2land=proc.time()-ptmd )
min_d2land[3]/60

#### SAVE d2land ####
saveRDS(env8_d2land, here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_d2land.rda')))



#### 3. GODAS-Temperature at 600m, Kelvin ####
...
for (i in 1:length(lon)) {
    ...
    new$survey <- sw$survey[i] #added survey number too

    godas=rbind(godas,new)
}
godas=godas[-1,]
names(godas) = c("date", "depth", "matched_lat", "matched_lon", paste0("potempK", dep), "true_lon", "true_lat", "effCells", "survey")

env3_godas600 <- godas


(min_t600=proc.time()-ptm )
min_t600[3]/60

#### SAVE temp600 ####
saveRDS(env3_godas600, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_temp600.rda') ))
...
# 
# #### SAVE wavepow ####
# saveRDS(env4_ww3, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_wavepow.rda') ))



#### 6 & 7. Slope and Aspect ####
#lon -180-180 for slope, aspect, ssh, sshsd, eke
...
for (i in 1:nrow(sw)){ 
  ...
  tmp$effCells <- sw$effCells[i]
    tmp$survey <- sw$survey[i] #added survey number too

  env67_slpasp <- rbind(env67_slpasp, tmp)

}

(min_slpasp=proc.time()-ptm )
min_slpasp[3]/60


#### SAVE slpasp ####
saveRDS(env67_slpasp, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_slpasp.rda') ))
...
#### 9. SSH ####
# More details on extracting the data from CMEMS are in '4_PmSSHEKE.rmd', but now don't have to switch over to it.
...
for (i in 1:nrow(sw)){
  ...
  sshtmp$effCells <- sw$effCells[i]
      sshtmp$survey <- sw$survey[i] #added survey number too

  env9_ssh <- rbind(env9_ssh, sshtmp)
}

(min_ssh=proc.time()-ptm )
min_ssh[3]/60

#### SAVE ssh ####
saveRDS(env9_ssh, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_ssh.rda') ))
...
for (i in 1:nrow(sw)){
  ...
  sshsdtmp$survey <- sw$survey[i] #added survey number too
  env10_sshsd <- rbind(env10_sshsd, sshsdtmp)
}

(min_sshsd=proc.time()-ptm )
min_sshsd[3]/60

#### SAVE sshsd ####
saveRDS(env10_sshsd, here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_sshsd.rda') ))
...
for (i in 1:nrow(sw)){
  ...
  eketmp$effCells <- sw$effCells[i]
    eketmp$survey <- sw$survey[i] #added survey number too

  env11_eke <- rbind(env11_eke, eketmp)
}

(min_eke=proc.time()-ptm )
min_eke[3]/60

#### SAVE eke ####
saveRDS(env11_eke, here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_eke.rda')))


total_min = proc.time()-ptmd
total_min[3]/60
```
