...
d3 <-aggregate(x = PmTallyLong$pa,                # Specify data column
          by = list(PmTallyLong$year,PmTallyLong$value, PmTallyLong$forage),              # Specify group indicator
          FUN = sum)
colnames(d3) <- c('year', 'type', 'forage', 'total')
...
ggplot(d3, aes(x=type, y=total, fill=forage)) + 
  geom_bar(stat="identity") + 
  facet_grid(~year) +
  theme_bw() +
   scale_fill_manual(values = c('#3b0f70','#f7705c', '#3b0f70'), name = "") +
  ...
  labs(x='Encounter Type', y = 'Total Encounters')
...
d2 <-aggregate(x = PmTally$pa,                # Specify data column
          by = list(PmTally$year,PmTally$name, PmTally$forage),              # Specify group indicator
          FUN = sum)
colnames(d2) <- c('year', 'type', 'forage', 'total')

ggplot(d2, aes(x=as.factor(year), y=total, fill=type)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  theme_bw()
