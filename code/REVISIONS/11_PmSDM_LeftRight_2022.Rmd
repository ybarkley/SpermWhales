...
PmLeft <- filter(PmCombo, peak == 'A' | sid < 999 | pa == 0)
PmLeft2 <- unique(bind_rows(PmLeft, PmOnePeak))

PmRight <- filter(PmCombo, peak == 'B' | sid < 999 | pa == 0)
...
PmLeftRight <- filter(PmLeftRight, loc == 1 | sid <999 | pa==0)  #still has doubles for loc encs
...
modLeft2  <- gam(pa ~  s(Longitude, Latitude, k=12) + s(SST, k=3) +s(SSHsd, k=3) +  offset(log.effort) , data = trainLeft, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modLeft2)
...
modRight2  <- gam(pa ~  s(Longitude, Latitude, k=12) +  s(SST, k=3) + s(SSHsd, k=3) +  offset(log.effort) , data = trainRight, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modRight2)
