...
pmDetsub <- filter(pmDets, survey == surveynum, type=='best' | loc == 0)  # includes both 'best' localized positions, sightings, and non-localized acoustic encounters
...
```{r}
#1641: the A326S242 SHOULD have effort calculated since both were ON along straight trackline

eff = 134387571
gridEffort$detections$effortArea[33] <- eff
#add appropriate effort into grid cell as needed, then rerun centroids
gridEffort$effort$area[17636] <- eff

centroidAll <- gridCentroid(gridEffort, gpsAll) #assigns the 'pa' column where 0 is no whale, 1 is whale
centroidAll[which(centroidAll$effCells==17636), ]$start <- gridEffort$detections[33,5]  #Give the centroid a start and end time, use the detection time
centroidAll[which(centroidAll$effCells==17636), ]$end <- gridEffort$detections[33,5]
centroidAll[which(centroidAll$effCells==17636), ]$diff_mins <- 0
```
...
pdf(here::here('figures', paste0('Grid', surveynum, '_', gridsize, 'km_', loctype, '_allEncs_2022.pdf')), width=11,height=8)
...
# check that all pa > 0 have effort > 0
all_prz <- dplyr::filter(centroidAll, pa>0) # pa should equal 1 at this point
ifelse( all_prz$EffArea > 0, 'TRUE', 'FALSE' ) #do all 
...
if(length(dupInd)==0){
  ...
  centroidAlltmp <- filter(centroidAll, pa != 1)     #remove the rows with single presences, pa == 1 - KEEP
