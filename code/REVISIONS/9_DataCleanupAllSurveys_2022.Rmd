...
surveynum = 1706  #load grids one survey at a time to save the detection info
gridsize = 16
loctype = 'Combined'
loctype2 = 'Comb'
...
gridEffortCleanup$gps$join.aeffort <- str_trim(gridEffortCleanup$gps$join.aeffort, side = "right")
...
#for 16km res
pmDetsGridTot16 = rbind.data.frame(pmDetsGrid1641, pmDetsGrid1303, pmDetsGrid1604, pmDetsGrid1705, pmDetsGrid1706)
saveRDS(pmDetsGridTot16, here::here( paste0('output/gridEffort/',  gridsize, ' km-', loctype, '/AllSurveys_', gridsize, 'km_',loctype2, '_AllDetectionsGrid.rda') ))
...
# colnames(env11_eke)[colnames(env11_eke) == "date"] <- "UTC" #2022: add column to ssh df
...
env1_sst[env1_sst[,10]=='NA'] <- NA      
env2_chla[env2_chla[,10]=='NA'] <- NA       

env3_godas600[env3_godas600[,5]=='NA'] <- NA  

env4_ww3[env4_ww3[,15]=='NA'] <- NA        
...
env9_ssh$ssh[env9_ssh[,11]=='NA'] <- NA     
env10_sshsd$sshsd[env10_sshsd[,9]=='NA'] <- NA
env9_ssh$eke[env9_ssh[,16]=='NA'] <- NA
env12_d2smt[env12_d2smt[,1]=='NA'] <- NA 
...
PmEnvTMP = data.frame(env1_sstSORT[, c(1:3, 5:10)], `chla`=env2_chlaSORT[, 9], `temp600`=env3_godasSORT[,5], `wavepow`=env4_ww3SORT[,15], `bath_m`= env5_bathSORT$bath_m, 'd2land'=env8_d2landSORT[,1], `ssh`=env9_sshSORT[,11], `sshsd`=env10_sshsdSORT[,9], `eke`=env9_sshSORT[,16], `d2smt`=env12_d2smtSORT[,1])
...
nrow(pa1) == nrow(pmDetsGridTot16)  ## !!!!!! Check number of presences: Do they match btwn centroid data and grid data?
...
PmEnv <- PmEnvTMP2 %>% drop_na() #remove NAs, which should only be from the absences  2022: no pa>0 had NAs
...
for (s in c(1641, 1303, 1604, 1705, 1706)){
 ...
 clkSub <- filter(pmDetsGridTot16, survey == s)  #encounter data with click info (no old env data)
  
 mergeSub <- merge(cenSub, clkSub, by.x = 'effCells', by.y = 'gridIndex') #click info is maintained even for encs that occur in the same cell, woo hoo! AND those with effortarea=0 are removed
...
merge_tmp2 <- dplyr::select(mergeClickDat, c(6,2,23,19:22,1,3,46,4,5,27,35,36,29, everything())) #This contains the presences with the env data from the centroid locations (6,2,24,20:23,1,3,46,4,5,44,28,29,37,36,45,30, everything())
...
merge_tmp3 <- merge_tmp3[,-17] #remove extra survey column


#REMOVE A999s
merge_tmp3 <- dplyr::filter(merge_tmp3, acid <999) #remove extra survey column
...
test <- readRDS(here::here(  paste0('output/envData/25 km-', loctype, '/CompletePm_25km_', loctype2, '_raw.rda')  ))
