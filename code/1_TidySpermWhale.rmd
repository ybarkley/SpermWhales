...
```{r message=F}
sperm.dat <- read_csv('./data/Spermies_LOCandSIGHT.csv') #older file, but mostly using it to get ID label
sperm.datsub <- sperm.dat[,c(3,5:7)]

sperm.datsub <- unite(sperm.datsub, "ID", Survey.Number, Acoustic.ID, Sighting.ID)
...
vid2 <-filter(vid, vid$Sighting.ID < 999)
vid3 <- unite(vid2, "ID", Survey.Number, Acoustic.ID, Sighting.ID)
pmTidyv <- cbind(vid2[1:3], vid3)
colnames(pmTidyv) <- c('survey', 'acid', 'sid', 'ID', 'UTC', 'lat', 'lon')
...
colnames(pmTidyun) <- c('survey', 'acid', 'sid', 'ID', 'UTC', 'lat', 'lon')
...
pmTidyLOCandSIGHT <- bind_rows(pmTidy, pmTidyv)
pmTidyLOCandSIGHT <- arrange(pmTidyLOCandSIGHT, survey, acid)



#combine all encounters
pmTidyALL <- bind_rows(pmTidy, pmTidyv, pmTidyun)
