...
write.csv(PmEdit, here::here('/output/CompletePm_25km_all_2021.csv'))
...
modCombo2Donly <- gam(pa ~ s(Longitude, Latitude, k=10) + offset(log.effort) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo2Donly)
...
#spatial term only
modAcOnly4 <- gam(pa ~ s(Longitude, Latitude, k=10) + offset(log.effort), data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly4)
...
```
#Acoustic contour plot
```{r}
getIt <- getViz(modAcOnlyBest)
getIt2 <- plot(sm(getIt, 1))
...
#Load dataframe from the 'output/model' folder
...
modVisOnlyc <- gam(pa ~ s(Longitude, Latitude, k=10) + offset(log.effort), data = trainVisOnly, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modVisOnlyc)
...
plot(modVisOnlyb, select=2, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='SST (ºC)', seWithMean = TRUE, cex.axis = 1.2, cex.lab = 1.5 )
...
points(hawaiiMap$Longitude2, hawaiiMap$Latitude, pch=20, cex = 0.5)
```
#Sighting contour plot
```{r}
getIt <- getViz(modVisOnlyb)
getIt2 <- plot(sm(getIt, 1))
