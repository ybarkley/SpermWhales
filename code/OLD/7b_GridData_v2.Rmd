---
title: "Revise Grid Effort: new detection function"
...
centroidAll <- readRDS(here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids',  surveynum, '_', gridsize, 'km_', loctype2, 'raw.rda')))
...
#examine possible covariates, such as array
boxplot(pmDets$distance~pmDets$array, xlab="Array", ylab="Distance (m)")

#array included as covariate
...
plot(dsmAc, nc = 12)
gof_ds(dsmAc) #a large p-value indicates a good fit of the model to the data. The points fall on the diagonal line fairly well. 


#detection function with ALL data, ie. localized acoustics and visual sightings
dsmAll <- Distance::ds(pmDets) 
plot(dsmAll)

#truncate by the last 5%
dsmAll_trnc <- Distance::ds(pmDets, truncation='5%')
plot(dsmAll_trnc)
gof_ds(dsmAll_trnc)

#cosine adjustment terms determined by different values of alpha (y-intercept)
#order 2 is full cosine curve, order 3 is 1.5 'cosine wiggles'. hn and hr models start with order 2.
#constraints restrict the model from decreasing and increasing again farther away, the model remains flat or decreasing
dsmAll_15k <- Distance::ds(pmDetsAc, truncation=15000) #half-normal, acoustics only
plot(dsmAll_15k)
dsmAll_15khz <- Distance::ds(pmDets, key = 'hr', truncation = 15000)
```
##REGRIDDING
```{r}
...
gridEffortC <- doAllGrid(gps = gpsAll,
                     bounds = NULL,
                     dets = pmDetsub,
                     trunc_m = 15e3, #METERS
                     dsmodel = dsmAll_15k, #NULL or dsm 
                     pixel = NULL,
                     grid = NULL,
                     plot = F)
...
centroidAllB <- gridCentroid(gridEffortB, gpsAll)


#Save raw grid and centroid data
saveRDS(gridEffort, here::here(paste0('output/gridEffort/', gridsize, ' km-', loctype), paste0('gridEffort', surveynum, '_', gridsize, 'km_', loctype2, '_rawB.rda')))

saveRDS(centroidAll, here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids', surveynum, '_', gridsize, 'km_', loctype2, '_rawB.rda')))
```
...
saveRDS(ALLcentroids, here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids', surveynum, '_', gridsize, 'km_', loctype2, '_raw.rda')))
```
...
```{r}
#1. check that all detections are included as expected. Should be all TRUE
...
#Plot Grid with Detections with No Effort
REMEMBER: GridCentJoin only includes the surrounding cells WITH effort, so not all 'Dir' may be included in the plot.
CHOOSE the replacement cells using the direction.
```{r}
x <- gridEffortB
...
pdf(here::here('figures', paste0('Grid', surveynum, '_', gridsize, 'km_', loctype, 'B.pdf')), width=11,height=8)
...
#Replace cells containing detections with no effort
```{r}
...
#Save new grid data: 
!!! All detections must have Effort > 0
```{r}
...
```{r}
##STOP!
# Did the original grid data include detections with no effort?
# If YES, then run gridCentroid again. If NO, then do not run it again.
centroidAll <- gridCentroid(gridEffort, gpsAll)
