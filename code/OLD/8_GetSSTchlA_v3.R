...
envDatWP = 'hawaii_soest_98bb_253a_eb1c'
httpWP = 'http://apdrc.soest.hawaii.edu/erddap/'


#1 of 4. Get dataset info
# edinfo <-browseEdinfo(var='sst') #use if the dataset is in the package
edinfoSST <- erddapToEdinfo(envDatSST, baseurl = httpSST)
edinfoCHLA <- erddapToEdinfo(envDatCHLA, baseurl = httpCHLA)
...
#SST
matchedData <- matchEnvData(centroidData, nc=edinfoSST,
...
#CHLA
matchedData <- matchEnvData(centroidData, nc=edinfoCHLA,
                            fileName = here::here('output/envData', paste0(gridsize, ' km-', loctype, '/',
                                                                           surveynum, "_", gridsize, 'km_',
                                                                           loctype2, '_', envType, ".nc")))
edinfo = edinfoCHLA
...
#WP
matchedData <- matchEnvData(centroidData, nc=edinfoWP,
                            fileName = here::here('output/envData', paste0(gridsize, ' km-', loctype, '/',
                                                                           surveynum, "_", gridsize, 'km_',
                                                                           loctype2, '_', envType, ".nc")))
...
if (envType == 'wp'){
  ...
  EnvData_tmp <- EnvData_tmp[, c(1:8, 33, 9:32)] #move wavepow to col 9 to match other datasets using other script
  saveRDS(EnvData_tmp, paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_10km_',loctype2, '_', envType, '.rda'))
...
#3. 
matchedData5 <- matchEnvData(matchedNA4[,1:8], nc=edinfo, buffer = c(gridsize/100+0.055,gridsize/100+0.1,0)) #use buffer to get data outside the grid size in either direction on same day
