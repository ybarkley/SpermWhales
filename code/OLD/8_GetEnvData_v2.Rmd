...
sst=sst[-1,]
# names(sst) = c("date", "matched_lat", "matched_lon", "sstAQm", "true_lon", "true_lat", "effCells")
...
# sst$true_lat <- lat
# sst$true_lon <- lon
# sst$diffdist <- distHaversine(cbind(sst$matched_lon, sst$matched_lat), cbind(sst$true_lon, sst$true_lat))
# sst$effcells <- sw$effCells
...
chla_mon=chla_mon[-1,]
names(chla_mon) = c("date", "matched_lat", "matched_lon", "matched_chla_mon", "true_lon", "true_lat", "effCells")
proc.time()-ptm
...
ww3=ww3[-1,]
names(ww3) = c("date", "matched_lat", "matched_lon", "whght", "wtime", "true_lon", "true_lat", "effCells")
...
for (i in 1:nrow(sw)){
  ...
  bath_match <- rbind(bath_match, btmp)
}

names(bath_match) = c("matched_lat", "matched_lon", "bath_m","true_lon", "true_lat", "effCells")

elapsedmin=proc.time()-ptm 
elapsedmin[3]/60 
#bathymetry data
# bathdat = do.call(rbind.data.frame, bath_match1)
...
for (i in 1:nrow(sw)){
  ...
  dist2land_match <- rbind(dist2land_match, dtmp)
}
