---
title: 'GAMs - Acoustics Only - 10 km '
...
require(mgcv)
tw10S999 <- gam(pa ~ s(bath_m, k=3) + s(dist, k=3) + s(d2smt, k=3) +  s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(ssh, k=3) + s(sshsd, k=3) + s(eke, k=3) + s(wavepow, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999)
```
```{r echo=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(tw10S999, pages = 3, residuals = FALSE, pch = 20, cex = 0.25,
scheme = 1, shade = T, shade.col = 'turquoise4', main='Full Ac Only-10km')
```
...
tw10S999b <- gam(pa ~ s(bath_m, k=3) + s(temp600, k=3) + s(ssh, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999b)

```
```{r echo=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(tw10S999b, pages = 3, residuals = FALSE, pch = 20, cex = 0.25,
scheme = 1, shade = T, shade.col = 'turquoise4', all.terms = TRUE, main='Reduced Ac Only-10km')
```
...
tw10S999c <- gam(pa ~ s(temp600, k=3) + s(ssh, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999c)

```
```{r echo=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(tw10S999c, pages = 3, residuals = FALSE, pch = 20, cex = 0.25,
scheme = 1, shade = T, shade.col = 'turquoise4', all.terms = TRUE, main='Reduced Ac Only-10km')
```
...
### Include 2D Lat-Lon
Includes 2D Lat-Lon smoother to account for spatial structure in the data  
    * Notice that the temperature at 600m is no longer significant compared to the previous model, same as 25 km models 
    * Depth, Distance to land and seamounts, chlorophyll, and SSH are now significant
      
```{r}

tw10S999LL <- gam(pa ~ s(Longitude, Latitude) + s(bath_m, k=3) + s(dist, k=3) + s(d2smt, k=3) +  s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(ssh, k=3) + s(sshsd, k=3) + s(eke, k=3) + s(wavepow, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999LL)
...
```{r echo=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(tw10S999LL, pages = 4, residuals = FALSE, pch = 20, cex = 0.25,
shade = T, scheme = 1, theta= 45, phi=40, shade.col = 'turquoise4', all.terms = TRUE, main='Acoustics Only')
```
```{r echo=FALSE, fig.cap = "Purple dots represent acoustically detected encounters."}
par(mfrow = c(1,1))
plot(tw10S999LL, select = 1, scheme = 2, lwd = 2, main='Spatial 2D Smoother') #select the first smoother, select = 1
...
# model diagnostics
par(mar=c(4,4,3,3),mfrow = c(2,2))
gam.check(tw10S999LL)
```
Residuals more normally distributed compared to 25 km model?
...
```{r eval =FALSE}
tw10S999LLb <- gam(pa ~ s(Longitude, Latitude) + s(bath_m, k=3) + s(dist, k=3) + s(d2smt, k=3) + s(chla, k=3) + s(ssh, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999LLb)
```


* Remove chlorophyll & distance to land

```{r}
tw10S999LLc <- gam(pa ~ s(Longitude, Latitude) + s(bath_m, k=3)  + s(d2smt, k=3) + s(ssh, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999LLc)
```

```{r echo=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(tw10S999LLc, pages = , residuals = FALSE, pch = 20, cex = 0.25,
shade = T, scheme = 1, theta= 30, phi=20, shade.col = 'aquamarine', all.terms = TRUE, main='Reduced Ac Only w/ 2D smoother-25km')
```

```{r echo=FALSE, fig.cap = "Purple dots represent acoustically detected encounters."}  
par(mfrow = c(1,1))
plot(tw10S999LLc, select = 1, scheme = 2, lwd = 2, main='2D Spatial Smoother') #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
whales <- subset(trainS999, pa >0) #get points for whales to plot
...
* Remove depth and SSH, leaving only distance to seamounts ???
```{r}
tw10S999LLd <- gam(pa ~ s(Longitude, Latitude) + s(d2smt, k=3) + offset(log.effort), data = trainS999, family = tw, link = 'log', select = TRUE, method = "REML")
summary(tw10S999LLd)
```

```{r echo=FALSE}
par(mar=c(4,4,3,3),mfrow = c(2,2))
plot(tw10S999LLd, pages = , residuals = FALSE, pch = 20, cex = 0.25,
shade = T, scheme = 1, theta= 30, phi=20, shade.col = 'aquamarine', all.terms = TRUE, main='Reduced Ac Only w/ 2D smoother-25km')
```

```{r echo=FALSE, fig.cap = "Purple dots represent acoustically detected encounters."}  
par(mfrow = c(1,1))
plot(tw10S999LLd, select = 1, scheme = 2, lwd = 2, main='2D Spatial Smoother') #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
whales <- subset(trainS999, pa >0) #get points for whales to plot
...
### Conclusions
Models for 10 km spatial scale explain the data with fewer variables compared to the 25 km models but the explained deviance is much lower for the 10 km plots, about half the explained deviance of the 25 km models.

Plots without a spatial smoother included temperature at 600 m and SSH as significant but very low explained deviance ~2.8%, yikes.
Including a spatial smoother reduced the model even further, excluding all dynamic variables and only including the spatial smoother with distance to seamount. This does not seem very informative? The spatial smoother does indicate a similar pattern in the probability of sperm whale occurrence with the general pattern of an overall higher occurrence north of the islands with the highest probability in the northwestern region. 
Are there generally more seamounts in this area that the 'distance to seamount' variable can pick up on using the finer spatial scale? 
