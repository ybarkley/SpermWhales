...
# sw=readRDS(here::here('Documents\\PHD\\Chp 3\\data\\centroids\\10 km\\centroids1705_10km.rda')) #using NOAA laptop
sw <- readRDS(here::here(paste0('output/centroids/', gridsize, ' km-', loctype, '/', 'centroids', surveynum, '_', gridsize, 'km_', loctype2, '.rda')))
# sw <- readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\centroids\\10 km\\centroids1604_10km.rda')
...
# minUTC <- ymd_hms(paste0('2017-10-01 00:00:00'))
...
# bathy_tot <- readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\bathy_tot.rda') #desktop
# bathy_tot <- readRDS(here::here('PHD\\Chp 3\\data\\bathy_tot.rda')) #NOAA
...
# dist_slice <-readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\dist2land.rda')  #desktop
# dist_slice <-readRDS(here::here('PHD\\Chp 3\\data\\dist2land.rda')  #NOAA
...
# slope_aspdf2 <-readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\slope_aspHIdf.rda') #desktop
# slope_aspdf2 <- readRDS(here::here('PHD\\Chp 3\\data\\slope_aspHIdf.rda')) #NOAA
...
sshtidyaTMP <-readRDS(here::here('data', 'ssh_tidy.rda')) #laptop
# sshtidya <-readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\ssh_tidy.rda') #desktop 
# sshtidya <- readRDS(here::here('PHD\\Chp 3\\data\\ssh_tidy.rda') #NOAA
sshtidya = dplyr::filter(sshtidyaTMP, date >= minUTC & date < maxUTC)
min(sshtidya$date)
max(sshtidya$date)

#10. SSHsd: same as ssh,  0 to 360
sshZtidyaTMP<-readRDS(here::here('data', 'sshsd_tidy.rda')) #laptop
# sshZtidya <-readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\sshsd_tidy.rda') #desktop
# sshZtidya <- readRDS(here::here('PHD\\Chp 3\\data\\sshsd_tidy.rda') #NOAA
sshZtidya= filter(sshZtidyaTMP, date >= minUTC & date < maxUTC)


#11. EKE: same as ssh, using U and V data,  0 to 360
ekeZtidyaTMP<-readRDS(here::here('data','eke_tidy.rda')) #laptop
# ekeZtidya <-readRDS('C:\\Users\\yvers\\Documents\\CHP 3\\eke_tidy.rda') #desktop
# ekeZtidya<-readRDS(here::here('Documents\\PHD\\Chp 3', 'eke_tidy.rda')) #NOAA
...
# env5_bath <- readRDS(here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_bathyAll.rda')))

```

##2. Evaluate the bathy data
Truncate at 700 m, the data support it.
```{r}
...
histogram(swdat3_pa1$bath_m, main = "Bathymetry of Whale", breaks=20)
histogram(swdat3_pa0$bath_m, main = "Bathymetry of Absences")

#keep the 1303 at 697 m at the actual sighting location of 1303.S71 dammit
swdat3_pa1[1, 8] <- -697
# 
tmp <- filter(swdat3_pa0, bath_m <= -700)
env5_bath2 = rbind(swdat3_pa1, tmp)
env5_bath = env5_bath2[, c(8:12,1)]


####
env5_bathAll <- env5_bath
tmp <- filter(env5_bath, bath_m <= -700)
env5_bath=tmp

#### SAVE subset of bathy data ####
saveRDS(env5_bath, here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2,'_bathy.rda')))

sw <-filter(sw, effCells %in% env5_bath$effCells)

saveRDS(sw, here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2,'_sw.rda')))

# sw<- readRDS(here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_sw.rda')))
```


```{r}
#for adding one row to 1303 (??? don't know why this is here... 9/12/20)
sw <-filter(sw, effCells == 4034)
...
# https://pae-paha.pacioos.hawaii.edu/erddap/griddap/ww3_global.csv?Tper[(2020-09-19T09:00:00Z):1:(2020-09-19T09:00:00Z)][(0.0):1:(0.0)][(-77.5):1:(77.5)][(0.0):1:(359.5)],Thgt[(2020-09-19T09:00:00Z):1:(2020-09-19T09:00:00Z)][(0.0):1:(0.0)][(-77.5):1:(77.5)][(0.0):1:(359.5)]
ptm=proc.time()
env4_ww3 = rep(NA,6)
for (i in 1:length(lon)) {
    url = paste("https://pae-paha.pacioos.hawaii.edu/erddap/griddap/ww3_global.csv?Tper[(",dates[i],"):1:(",dates[i],")][(", lat[i], "):1:(", lat[i], ")][(", lon[i], "):1:(", lon[i], ")],Thgt[(",dates[i],"):1:(",dates[i],")][(", lat[i], "):1:(", lat[i], ")][(", lon[i], "):1:(", lon[i], ")]", sep = "")
    new = read.csv(url, skip = 2, header = FALSE)
    new$true_lon <- lon[i]
    new$true_lat <- lat[i]
    new$effCells <- sw$effCells[i]
