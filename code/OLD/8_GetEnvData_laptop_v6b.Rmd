...
maxUTC <- ymd_hms(paste0(2018,'-',01,'-',maxDay, ' 00:00:00'))  #Use this if max dates in Dec
...
Truncate at 1000 m if data support it.
```{r}
...
tmp <- filter(env5_bath, bath_m < -1000)
env5_bath=tmp
...
## 700 m EDIT
Sept 9, 2020- Adding in an extra step to extract the data for depths >=700 & <=1000. 
This is not necessary for future extractions.
```{r}

bathy700 <- readRDS(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_bathyAll700.rda'))
sw <-filter(sw, effCells %in% bathy700$effCells)
saveRDS(sw, here::here(paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2,'_sw.rda')))
...
for (i in 1:nrow(sw) ){ #1:nrow(sw)
  ...
  dtmp$effCells <- sw$effCells[i]
    env8_d2landWTF <- rbind(env8_d2landWTF, dtmp)
