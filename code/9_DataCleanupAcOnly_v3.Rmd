...
for (s in c(1641, 1303, 1604, 1705, 1706)){
 ...
 cenSub <- filter(PmEnvPres, survey == s)
 clkSub <- filter(pmDetsGridTot25, survey == s)
 ...
 mergeClickDat = rbind(mergeClickDat, mergeSub)  
...
#filter for depths < -700
trythis2 <- subset(trythis, bath_m <= -700)
...
# CompleteTotal <- readRDS( paste0('output/envData/', gridsize, ' km-', loctype, '/', surveynum, '_', gridsize, 'km_', loctype2, '_complete') )
```
...
CompleteScaled <- data.frame(dplyr::select(CompleteTotal, pa:distance), CompleteScaledtmp, dplyr::select(CompleteTotal, peak:array))
