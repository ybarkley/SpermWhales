...
scheme = 1, shade = T, shade.col = 'orange', all.terms = TRUE, main='NegBin Full Foragers')
```
...
scheme = 1, shade = T, shade.col = 'hotpink', all.terms = TRUE, main='Tweedie Full Combined-25km')
```
...
nbHuntb <- gam(pa ~ s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(sshsd, k=3) + offset(log.effort), data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbHuntb)
...
plot(nbHuntb, residuals = FALSE, pch = 20, cex = 0.25, ylim = c(-6,2),
scheme = 1, shade = T, shade.col = 'hotpink', all.terms = TRUE, main='NegBin Reduced Combined-25km')
...
```{r}
nbHuntLL <- gam(pa ~ s(Longitude, Latitude) + s(bath_m, k=3) + s(dist, k=3) + s(d2smt, k=3) +  s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(ssh, k=3) + s(sshsd, k=3) + s(eke, k=3) + s(wavepow, k=3) + offset(log.effort), data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbHuntLL)
AIC(nbHuntLL)
```
```{r echo=FALSE}
...
shade = T, scheme =2, theta= 45, phi=40, shade.col = 'hotpink', all.terms = TRUE)
...
### REDUCE MODEL PARAMETERS  
   ...
   * Keep:   
      ...
      + SSHsd
```{r echo=FALSE, eval=FALSE}
nbHuntLLb <- gam(pa ~ s(Longitude, Latitude) + s(sst, k=3) + s(chla, k=3) + s(temp600, k=3) + s(sshsd, k=3) + offset(log.effort), data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbHuntLLb)
```
...
nbHuntLLb2 <- gam(pa ~ s(Longitude, Latitude) + s(SST, k=3) + s(Chla, k=3) + s(Temp600m, k=3) + s(SSHsd, k=3) + offset(log.effort), data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(nbHuntLLb2)
```
```{r echo=FALSE}
...
plot(nbHuntLLb2, select=2, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'orange', all.terms = TRUE, xlab='Chl a (mg m-3)' , main='Best Set 2 Models w/Spatial Smoother-Reduced')

plot(nbHuntLLb2, select=3, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'orange', all.terms = TRUE, xlab='Temp at 600 m (°C)', main='Best Set 2 Models w/Spatial Smoother-Reduced')

plot(nbHuntLLb2, select=4, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'orange', all.terms = TRUE, xlab='SSHsd (m)', main='Best Set 2 Models w/Spatial Smoother-Reduced')
