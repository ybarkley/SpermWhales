...
matchedData <- matchEnvData(centroidData, nc=edinfo, fileName = here::here('output/envData', paste0(surveynum, "_", gridsize, 'km_',
                                                                                                        envType, '_', loctype2, ".nc")))
...
matchedDataComplete <- matchedData[complete.cases(matchedData[,9]),]  #make dataframe of complete rows in variable column
    ...
    matchedData2 <- matchEnvData(matchedNA[,1:8], nc=edinfo, buffer = c(gridsize/100+0.01,gridsize/100+0.01,0)) #use buffer to get data outside the grid size in either direction on same day
...
saveRDS(EnvData_tmp, here::here('output/envData', paste0(surveynum, '_10km_', envType, '_',loctype2, '.rda')))
