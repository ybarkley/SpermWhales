...
PmEditsubby <- subset( PmEdit, pa==0 | pa == 1 ) #sweet, works
...
#save for Acoustics Only Models
...
saveRDS(trainComb, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_CombA999.rda')  ))
saveRDS(testComb, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_CombA999.rda')  ))
...
saveRDS(trainAcOnly, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_S999D.rda')  ))
saveRDS(testAcOnly, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_S999D.rda')  ))
...
modCombo2Db <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3), data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo2Db)
...
```{r echo = FALSE}
require(mgcv)
modComboBest <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
...
plot(modComboBest, select=3, residuals = FALSE, pch = 20, shade = T, ylim = c(-2,2),
     scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='Temp at 584 m (°C)', seWithMean = TRUE, cex.axis = 1.2,  cex.lab = 1.5)
# axis(1,cex.axis=1.2)
# axis(2,cex.axis=1.2)

plot(modComboBest, select=4, residuals = FALSE, pch = 20, shade = T, ylim = c(-2,2),
...
plot(modComboBest, select = 1, scheme = 2, lwd = 2, cex.axis = 1.2,  cex.lab = 1.5)#, main='Best Set 2 Model w/Spatial Smoother-Reduced') #select the first smoother, select = 1
...
par(mfrow = c(1,1),mar=c(5,5,1,1))
plot(modAcOnlyBest, select = 1, scheme = 2, lwd = 2, cex.axis = 1.2, cex.lab = 1.5 ) #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
...
```{r echo=FALSE}
saveRDS(PmEdit, here::here(  paste0('output/models/',loctype, '/data/SighingOnly_25km_Combined.rda') ))
### SIGHTING DATA ONLY ####
survey = 'AllSurveys'
gridsize = 25
loctype = 'Combined'
...
#pa = 3 needs to be changed to pa =2 and delete the extra row
...
nrow(subset(PmVisDupAdd, acid==999))
nrow(PmVisDupAdd)
...
trainA999 <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_CombA999.rda')  ))
testA999 <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_CombA999.rda')  ))

colnames(trainA999)[30] <- 'SST'
colnames(trainA999)[57] <- 'log.Chla'
colnames(trainA999)[32] <- 'Temp584m'
colnames(trainA999)[34] <- 'Depth'
colnames(trainA999)[38] <- 'SSH'
colnames(trainA999)[39] <- 'SSHsd'


colnames(testA999)[30] <- 'SST'
colnames(testA999)[57] <- 'log.Chla'
colnames(testA999)[32] <- 'Temp584m'
colnames(testA999)[34] <- 'Depth'
colnames(testA999)[38] <- 'SSH'
colnames(testA999)[39] <- 'SSHsd'



modA999 <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort), data = trainA999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modA999)

modA999b <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort), data = trainA999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modA999b)
```
...
plot(modA999b, select=2, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='Depth (m)', seWithMean = TRUE )

plot(modA999b, select=3, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab=expression("Chl-a (mg/m"^3*")"), seWithMean = TRUE )

plot(modA999b, select=4, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='Temp at 584 m (ºC)', seWithMean = TRUE )

plot(modA999b, select=5, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='SSHsd (m)', seWithMean = TRUE )
...
```{r echo=FALSE, fig.cap = "Acoustics-Only Model 2: Contour plot of spatial smoother. Purple dots represent acoustically detected sperm whale encounters. Black dots are all data points with effort included in the model.", fig.width=6, fig.height=4}  
par(mfrow = c(1,1))
plot(modA999b, select = 1, scheme = 2, lwd = 2) #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
whales <- subset(trainA999, pa >0) #get points for whales to plot
...
#### PREDICT SIGHTHING ####
modVisResid<- trainA999 %>% mutate(resid = resid(modA999b), predict = predict(modA999b))
modVisPred <- predict.gam(modA999b, type = 'response')  #calculate MSE for these to compare with test set. If they're super different, speaks to the genrality of the model
...
modVisPredNew <- predict.gam(modA999b, newdata = testA999, type = 'response', se.fit = TRUE)
modVisPredNew2 <- data.frame(testA999, fit = modVisPredNew$fit, se.fit=modVisPredNew$se.fit)
modVisMSEtest <- mean((modVisPredNew2$pa - modVisPredNew2$fit)^2) #MSE

# mean(abs((testFinal$pa - testFinal$fit))) #Mean absolute error

# AIC
modVisAIC <- AIC(modA999b)

# Explained Deviance
modVisExp = round(((modA999b$null.deviance-modA999b$deviance)/modA999b$null.deviance)*100, 2)
...
# enter info by row
table[1,] <- c("Acoustics Only Model", paste0(modVisExp, '%'), round(modVisAIC,2), round(modVisMSEtrn,3), round(modVisMSEtest,3), paste0('Depth, SSH, LON:LAT'))
require(knitr)
kable(table, caption = "Performance Metrics for Combined Models")
```
