...
#### This document summarizes the results of the sperm whale distribution models for FORAGING WHALE GROUPS.
...
PmEditsubby <- filter( PmEdit, pa==0 | pa == 1 ) #sweet, works
...
modCombo <- gam(pa ~ s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo) 
...
plot(modComboBest, select=3, residuals = FALSE, pch = 20, shade = T, ylim = c(-2,2),
     scheme = 2, shade.col = 'lightcyan', all.terms = TRUE, xlab='Temp at 584 m (°C)', seWithMean = TRUE )
...
par(mfrow = c(1,1))
plot(modComboBest, select = 1, scheme = 2, lwd = 2)#, main='Best Set 2 Model w/Spatial Smoother-Reduced') #select the first smoother, select = 1
...
plot(modAcOnlyBest, select=2, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'honeydew2', all.terms = TRUE, xlab='Depth (m)', seWithMean = TRUE )

plot(modAcOnlyBest, select=3, residuals = FALSE, pch = 20, cex=0.001, shade = T, scheme = 2, shade.col = 'honeydew2', all.terms = TRUE, xlab='SSH (m)', seWithMean = TRUE )
...
par(mfrow = c(1,1))
plot(modAcOnlyBest, select = 1, scheme = 2, lwd = 2) #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
...
# enter info by row
table[1,] <- c("Combined Model", paste0(nbExpDev, '%'), round(nbAIC, 2), round(nbMSEtrain,3), round(nbMSEtest,3), paste0('Depth, Temp at 584m, SSHsd, Yr. Qtr, LON:LAT'))
