---
title: "Sperm Whale Habitat Model: Environmental Data"
...
sw <- read.csv(here::here('output', 'Spermies_20200424.csv'))
...
ncname = paste0(here::here('data'), '/', flist[7])
...
#Match slope and aspect with sperm whale locations
```{r}
...
for (i in 1:length(lon)) {
  # dates[i]
    #print(paste("i=", i, " n=", length(lon)))
    urlww3 = paste("http://apdrc.soest.hawaii.edu/erddap/griddap/hawaii_soest_98bb_253a_eb1c.csv?htsgwsfc[(",dates[i],"):1:(",dates[i],")][(", lat[i], "):1:(", lat[i], ")][(", lon[i], "):1:(", lon[i], ")],perpwsfc[(",dates[i],"):1:(",dates[i],")][(", lat[i], "):1:(", lat[i], ")][(", lon[i], "):1:(", lon[i], ")]", sep = "")
    newww3 = read.csv(urlww3, skip = 2, header = FALSE)
    
    ww3=rbind(ww3, newww3)
}
...
require(openxlsx)
nmintmp <- read.xlsx(paste0(here('data'), '/', 'SpermiesBath.xlsx'), sheet=3)
...
#since 'slope' is not saved...reload and cbind ww3
SwEnvData_tot <- cbind(SwEnvData_tot, ww3$`wavepow`)
colnames(SwEnvData_tot)[31] <- 'wavepow'

write.csv(SwEnvData_tot, 'C:\\Users\\yvers\\Documents\\CHP 3\\SpermWhales\\output/SpermiesWithEnvData_20200326.csv',  row.names = F)
```
