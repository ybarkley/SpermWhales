...
```{r}
PmCombo$click_code[PmCombo$ID == "1303_42_21"] <- "cdckrs"
PmCombo$click_code[PmCombo$ID == "1604_30_999"] <- "rs"
PmCombo$click_code[PmCombo$ID == "1604_67_999"] <- "rs"
PmCombo$click_code[PmCombo$ID == "1705_251_999"] <- "rs"
PmCombo$click_code[PmCombo$ID == "1706_2_999"] <- "rs"
PmCombo$click_code[PmCombo$ID == "1706_41_999"] <- "rs"
PmCombo$click_code[PmCombo$ID == "1706_135_999"] <- "ckrs" #not included as separate encounter, was combined with 1706A49 in same cell

Pm1303 <- filter(PmCombo, survey == 1303, acid < 999)
Pm1706 <- filter(PmCombo, survey == 1706, acid < 999)
Pm1706b <- filter(CompleteScaled, survey == 1706, acid < 999)
...
saveRDS(trainComb, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_Comb3.rda')  ))
saveRDS(testComb, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_Comb3.rda')  ))
...
saveRDS(trainForg, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_Hunt3.rda')  ))
saveRDS(testForg, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_Hunt3.rda')  ))
...
saveRDS(trainNonFor, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_NonFor2.rda')  ))
saveRDS(testNonFor, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_NonFor2.rda')  ))
...
#seed 1
trainHunt <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Train_',   gridsize, 'km_', loctype2, '_Hunt3.rda')  ))
testHunt <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Test_',     gridsize, 'km_', loctype2, '_Hunt3.rda')  ))
...
#seed 1
trainNonFor <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Train_',   gridsize, 'km_', loctype2, '_NonFor.rda')  ))
testNonFor <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Test_',     gridsize, 'km_', loctype2, '_NonFor.rda')  ))
...
modForage <- gam(pa ~  s(Longitude, Latitude, k=10) + s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML") #s(year, k=3) + qtr2
summary(modForage) 
...
modForage3 <- gam(pa ~  s(Longitude, Latitude, k=10) + s(Temp584m, k=3) + s(SSHsd, k=3) +s(log.Chla, k=3) +s(qtr, k=3)+ offset(log.effort) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForage3) 
...
modForageTW <- gam(pa ~  s(Longitude, Latitude, k=10) + s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainHunt, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modForageTW) 
...
# par(mar=c(4,4,1,1))
# 
# plot(modForageBest, select = 1, scheme = 2, lwd = 2,  cex.axis = 1.2, cex.lab = 1.5) #select the first smoother, select = 1
...
plot(modForageBest, select=2, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='Temp at 584 m (°C)' , seWithMean = TRUE, ylim = c(-2,2),cex.axis = 1.2, cex.lab = 1.5)#, shift = coef(modForageBest)[1],	trans	= function(x)	exp(x))

plot(modForageBest, select=4, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'gray85', all.terms = TRUE, xlab='SSHsd (m)',seWithMean = TRUE, ylim = c(-2,2),cex.axis = 1.2, cex.lab = 1.5 )
...
plot(modForageBest, select = 1, scheme = 2, lwd = 2, cex.axis = 1.2, cex.lab = 1.5, main='') #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
whales <- subset(trainHunt, pa >0) #get points for whales to plot

#for 2D smoother plot
points(whales$Longitude, whales$Latitude, pch = 16, col='mediumslateblue', 
       lwd=1.25, cex=1)
points(hawaiiMap$Longitude2, hawaiiMap$Latitude, pch=20, cex = 0.5)
```

### Foraging spatial smoother
```{r}
# vis.gam(modForageBest, view = c("Longitude", "Latitude"), plot.type = "contour", contour.col = "blue", color = "terrain")

getIt <- mgcViz::getViz(modForageBest)
...
```{r echo=FALSE, eval=FALSE}
## START HERE WITH 2D SMOOTH
modNonFor <- gam(pa ~  s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainNonFor, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modNonFor) 
...
modNonFor3 <- gam(pa ~  s(Longitude, Latitude, k=10) + offset(log.effort) , data = trainNonFor, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modNonFor3) 

modNonFor4 <- gam(pa ~  s(Longitude, Latitude, k=10) + s(Temp584m, k=3)+offset(log.effort) , data = trainNonFor, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modNonFor4) 

AIC(modNonFor, modNonFor2, modNonFor3, modNonFor4)

## Tweedie
modNonForTW <- gam(pa ~  s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainNonFor, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modNonForTW) 

modNonForTW2 <- gam(pa ~  s(Longitude, Latitude, k=10) + s(Depth, k=3) + offset(log.effort) , data = trainNonFor, family = tw, link = 'log', select = TRUE, method = "REML")
summary(modNonForTW2)

par(mar=c(4,4,3,2),mfrow = c(2,2))
gam.check(modNonFor2)
gam.check(modNonForTW2)
AIC(modNonFor2, modNonForTW2)
...
require(mgcv)
modNonForBest <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + offset(log.effort), data = trainNonFor, family = nb, link = 'log', select = TRUE, method = "REML")

# summary(modNonForBest)
...
table2[1,] <- c("Foraging", paste0(16.04, '%'), 677.37, round(0.03002856,3), round(0.02922875,3), 
              paste0('Temp at 584 m, log.Chla, SSHsd, LON:LAT'))
