...
#By Yvonne Barkley
gridCentroid <- function(x, gps) {
    
    effCells <- which(x$effort$area>0)
    dets <- x$detections$gridIndex
    
    result = NULL
    for (eff in effCells){
        #What is the effort area for each cell?
        EffArea <- x$effort$area[eff]
        #Get lat lon of centroid using the grid coordinates
        gridTest<-x$grid[[eff]]
        ct <- st_centroid(gridTest)
        #Are there whales in this cell (is eff found in dets)? Mark presences and absences (1 or 0)
        if (eff %in% dets == TRUE){ 
            pa = 1
        } else {
            pa = 0
        }
        #Get first and last times for each cell with effort
        UTC_start <- x$effort$first[eff]
        ...
        ctdf <- data.frame(effCells=eff, `EffArea` = EffArea, `start`=UTC_start,
                           `end`=UTC_end, `diff_mins`=UTC_diff, lon=ct[1], lat=ct[2], pa=pa)
