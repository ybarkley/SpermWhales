...
9/26/20
Choosing the side with the deepest depth values is just as arbitrary, or less, than choosing the midpoint of trackline segments for environmental variables. Most of the depth data were very similar between sides except for 4 or so values. Coupling this with the plots of the environmmental variables vs mean distance of detected animals showed that most depth values fell below 500 m difference between sides, with 4 outliers with a difference greater than 500 m.
...
plot(lrbestyay$m, abs(lrbestyay$diffbath), main="Diff in Depth over Distance", xlab = 'Mean Distance (km)', ylab = 'Diff in Depth (m)')
...
```{r}
##### Slope #####
plot(lrbestyay$m, abs(lrbestyay$diffslp), main = 'Diff in Slope over Distance', xlab = 'Mean Distance (km)', ylab = 'Diff in Slope (deg)')
...
plot(lrbestyay$m, abs(lrbestyay$diffasp), main = 'Diff in Aspect over Distance',xlab = 'Mean Distance (km)', ylab = 'Diff in Aspect (deg)')
...
#PLOT ALL DIFFERENCE vs DISTANCE plots
```{r}
require(Cairo)
Cairo(here::here('figures', paste0('LeftVsRIght_DiffInDists2.png')), width=10,height=10, units='in', dpi=300, bg='white')

par(mfrow=c(4,3),mar=c(4,4,3,3))

plot(lrbestyay$m, abs(lrbestyay$diffbath),  xlab = 'Mean Distance (km)', ylab = 'Diff in Depth (m)')
lmbest <- lm(abs(lrbestyay$diffbath) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffbath))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffbath))*0.82, paste0( 'p = ',round(pval,3)))

abline(coefficients(lmbest))
# summary(lmbest)
# plot(fitted(lmbest), resid(lmbest))
# abline(0,0)
# qqnorm(resid(lmbest))
# qqline(resid(lmbest))
# plot(density(resid(lmbest)))

plot(lrbestyay$m, abs(lrbestyay$diffdland),  xlab = 'Mean Distance (km)', ylab = 'Diff in Dist2Land (km)')
lmbest <- lm(abs(lrbestyay$diffdland) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffdland))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffdland))*0.81, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))
# summary(lmbest)

plot(lrbestyay$m, abs(lrbestyay$diffslp),   xlab = 'Mean Distance (km)', ylab = 'Diff in Slope (deg)')
lmbest <- lm(abs(lrbestyay$diffslp) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffslp))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffslp))*0.81, paste0( 'p = ',round(pval,3)))

abline(coefficients(lmbest))


plot(lrbestyay$m, abs(lrbestyay$diffasp),   xlab = 'Mean Distance (km)', ylab = 'Diff in Aspect (deg)')
lmbest <- lm(abs(lrbestyay$diffasp) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffasp))*0.99, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffasp))*0.92, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))


plot(lrbestyay$m, abs(lrbestyay$diffdmst),  xlab = 'Mean Distance (km)', ylab = 'Diff in Dist2Seamt(km)')
lmbest <- lm(abs(lrbestyay$diffdmst) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffdmst))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffdmst))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$diffsst),   xlab = 'Mean Distance (km)', ylab = 'Diff in SST (°C)')
lmbest <- lm(abs(lrbestyay$diffsst) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffsst))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffsst))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$chla_m),    xlab = 'Mean Distance (km)', ylab = 'Diff in Chl a (mg m-3)')
lmbest <- lm(abs(lrbestyay$chla_m) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$chla_m))*0.95, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$chla_m))*0.9, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$difft600),  xlab = 'Mean Distance (km)', ylab = 'Diff in Temp at 584m (°C)')
lmbest <- lm(abs(lrbestyay$difft600) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$difft600))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$difft600))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$diffssh),   xlab = 'Mean Distance (km)', ylab = 'Diff in SSH (m)')
lmbest <- lm(abs(lrbestyay$diffssh) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffssh))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffssh))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$diffsshsd), xlab = 'Mean Distance (km)', ylab = 'Diff in SSHsd (m)')
lmbest <- lm(abs(lrbestyay$diffsshsd) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffsshsd))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffsshsd))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$diffeke),   xlab = 'Mean Distance (km)', ylab = 'Diff in EKE (m/s)')
lmbest <- lm(abs(lrbestyay$diffeke) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffeke))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffeke))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))

plot(lrbestyay$m, abs(lrbestyay$diffwp),    xlab = 'Mean Distance (km)', ylab = 'Diff in Wave Power (kW/m)')
lmbest <- lm(abs(lrbestyay$diffwp) ~ lrbestyay$m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffwp))*0.9, paste0( 'R2 = ',round(r2,2)))
text(min(lrbestyay$m)+2, max(abs(lrbestyay$diffwp))*0.82, paste0( 'p = ',round(pval,3)))
abline(coefficients(lmbest))
dev.off()
```
...
diffvars <- as.data.frame(bathID,d2smtID, ekeID, sshsdID,slpID, wpID)
#make vector of IDs that have outliers
...
#PLOT LEFT vs RIGHT
```{r}
#Remember, using scaled data - not for plots tho 9/27/20
require(Cairo)
Cairo(here::here('figures', paste0('LeftVsRIght_LinMods2.png')), width=10,height=10, units='in', dpi=300, bg='white')
par(mfrow=c(4,3),mar=c(4,4,3,3))

plot(sw.bestA2$bath, sw.bestB2$bath, xlab = 'Depth (m), Left', ylab ='Depth (m), Right')
lmbest <- lm(sw.bestA2$bath ~ sw.bestB2$bath)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$bath)*2, min(sw.bestB2$bath), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$bath)*2, min(sw.bestB2$bath)*0.9, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$d2land_km, sw.bestB2$d2land_km, xlab = 'Dist2Land (km), Left', ylab ='Dist2Land (km), Right')
lmbest <- lm(sw.bestA2$d2land_km ~ sw.bestB2$d2land_km)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2land_km)*2, min(sw.bestB2$d2land_km), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$d2land_km)*2.5, min(sw.bestB2$d2land_km)*0.9, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$slp_deg, sw.bestB2$slp_deg, xlab = 'Slope (deg), Left', ylab ='Slope (deg), Right')
lmbest <- lm(sw.bestA2$slp_deg ~ sw.bestB2$slp_deg)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$slp_deg)*0.8, min(sw.bestB2$slp_deg), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$slp_deg)*0.9, min(sw.bestB2$slp_deg)*9, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$asp_deg, sw.bestB2$asp_deg,  xlab = 'Aspect (deg), Left', ylab ='Aspect (deg), Right')
lmbest <- lm(sw.bestA2$asp_deg ~ sw.bestB2$asp_deg)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$asp_deg)*0.8, min(sw.bestB2$asp_deg), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$asp_deg)*0.8, min(sw.bestB2$asp_deg)*8, paste0( 'r = ',0))

plot(sw.bestA2$d2smt_km, sw.bestB2$d2smt_km, xlab = 'Dist2Seamt (km), Left', ylab ='Dist2Seamt (km), Right')
lmbest <- lm(sw.bestA2$d2smt_km ~ sw.bestB2$d2smt_km)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$d2smt_km)*0.85, min(sw.bestB2$d2smt_km)*40, paste0( 'r = ',round(sqrt(r2),2)))


plot(sw.bestA2$sstAQ_m, sw.bestB2$sstAQ_m,   xlab = 'SST (°C), Left', ylab ='SST (°C), Right')
lmbest <- lm(sw.bestA2$sstAQ_m ~ sw.bestB2$sstAQ_m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$sstAQ_m)*0.93, min(sw.bestB2$sstAQ_m), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$sstAQ_m)*0.97, min(sw.bestB2$sstAQ_m)*1.025, paste0( 'r = ',round(sqrt(r2),2)))


plot(sw.bestA2$chla_m, sw.bestB2$chla_m,     xlab = 'Chl a (mg m-3), Left', ylab ='Chl a (mg m-3), Right')
lmbest <- lm(sw.bestA2$chla_m ~ sw.bestB2$chla_m)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$chla_m)*0.9, min(sw.bestB2$chla_m), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$chla_m)*0.9, min(sw.bestB2$chla_m)*1.13, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$temp600C, sw.bestB2$temp600C, xlab = 'Temp at 584m (°C), Left', ylab ='Temp at 584m (°C), Right')
lmbest <- lm(sw.bestA2$temp600C ~ sw.bestB2$temp600C)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$temp600C)*0.93, min(sw.bestB2$temp600C), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$temp600C)*0.97, min(sw.bestB2$temp600C)*1.025, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$ssh, sw.bestB2$ssh, xlab = 'SSH (m), Left', ylab = 'SSH (m), Right')
lmbest <- lm(sw.bestA2$ssh ~ sw.bestB2$ssh)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$ssh)*0.91, min(sw.bestB2$ssh), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$ssh)*0.91, min(sw.bestB2$ssh)*1.06, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$sshsd, sw.bestB2$sshsd, xlab = 'SSHsd (m), Left', ylab = 'SSHsd (m), Right')
lmbest <- lm(sw.bestA2$sshsd ~ sw.bestB2$sshsd)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$sshsd)*0.8, min(sw.bestB2$sshsd), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$sshsd)*0.8, min(sw.bestB2$sshsd)*2.4, paste0( 'r = ',round(sqrt(r2),2)))


plot(sw.bestA2$eke, sw.bestB2$eke,     xlab = 'EKE (m/s), Left', ylab = 'EKE (m/s), Right')
lmbest <- lm(sw.bestA2$eke ~ sw.bestB2$eke)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$eke)*0.8, min(sw.bestB2$eke), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$eke)*0.8, min(sw.bestB2$eke)*18, paste0( 'r = ',round(sqrt(r2),2)))

plot(sw.bestA2$wavepow, sw.bestB2$wavepow,  xlab = 'Wave Power (kW/m), Left', ylab = 'Wave Power (kW/m), Right')
lmbest <- lm(sw.bestA2$wavepow ~ sw.bestB2$wavepow)
pval = round(summary(lmbest)$coefficients[2,4],5)
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$wavepow)*0.88, min(sw.bestB2$wavepow), paste0( 'pval=',round(pval,2)))
text(max(sw.bestA2$wavepow)*0.88, min(sw.bestB2$wavepow)*3, paste0( 'r = ',round(sqrt(r2),2)))
...
```{r}
cor(sw.bestA2$sstAQ_m, sw.bestB2$sstAQ_m)
...
plot(sw.bestA2$d2smt_km, sw.bestB2$d2smt_km)
mdl <- lm(sw.bestA2$d2smt_km ~ sw.bestB2$d2smt_km)
pval = round(summary(mdl)$coefficients[2,4],5)
r2 = summary(mdl)$adj.r.squared
...
#From data exploration in GAMS code
...
Cairo(here::here('figures', paste0('DataPlotsScaled_', gridsize, 'km_', loctype,'_', colnames(datPlot)[2], '.png')), width=14,height=6, units='in', dpi=300, bg='white')
