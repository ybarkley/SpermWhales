...
```{r}
require(Cairo)
Cairo(here::here('figures', paste0('LeftVsRIght_DiffInDists.png')), width=10,height=10, units='in', dpi=300, bg='white')
...
text(max(lrbestyay$m)*0.95, min(abs(lrbestyay$diffbath))*20, paste0( 'R2 = ',round(r2,2)))
...
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(max(lrbestyay$m)*0.95, min(abs(lrbestyay$diffdland))*20, paste0( 'R2 = ',round(r2,2)))
abline(coefficients(lmbest))
summary(lmbest)
...
r2 = summary(lmbest)$adj.r.squared
# text(max(sw.bestA2$d2smt_km)*0.75, min(sw.bestB2$d2smt_km), paste0( 'pval=',round(pval,2)))
text(max(lrbestyay$m)*0.95, min(abs(lrbestyay$diffdmst))*20, paste0( 'R2 = ',round(r2,2)))
abline(coefficients(lmbest))
...
plot(lrbestyay$m, abs(lrbestyay$difft600),  xlab = 'Mean Distance (km)', ylab = 'Diff in Temp at 600m (°C)')
...
plot(sw.bestA2$temp600C, sw.bestB2$temp600C, xlab = 'Temp at 600m (°C), Left', ylab ='Temp at 600m (°C), Right')
