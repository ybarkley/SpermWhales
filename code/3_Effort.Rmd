...
Use the 'str8sub' data because those include only the straight 
```{r}
survey = 1641
...
eff$PCTime %in% str8segs$UTC
eff[duplicated(eff),]
eff[duplicated(eff$PCTime),]
# dup <- as.data.frame(table(df2$UTC))
# dup[dup$Freq >1,]
 
df1 <- as.data.frame(as.POSIXct(str8segs$UTC ))
colnames(df1) <- "UTC"

df2 <- data.frame(as.POSIXct(eff$PCTime), eff$ac_effort, eff$vis_effort)
colnames(df2) <- c("UTC", "aeffort", "veffort")
...
# what3 <- filter(test, straight == 'TRUE' & ac_effort == 'on')
...
write.csv(str8eff, here::here('output', paste0('straightPathWeffort_', survey, '.csv')), row.names = FALSE)
...
indx <- setDT(reference)[data, roll = "nearest", which = TRUE, on = "datetime"]


take29 <-as.data.frame(setDT(effdates)[alldates, effort, roll = Inf, on = "UTC"])
indx <- setDT(effdates)[alldates, roll = Inf, which = TRUE, on = "UTC"]
alldates$new <- indx


#does not match to nearest, lots are missing
test <- setDT(alldates)[effdates, c("effort") := .(effort), on=c("UTC")]
test2 <- test %>% fill(effort, .direction = "down") %>% na.omit()

what <- setDT(str8sub1303)[eff, c("ac_effort") := .(effort), on=c("UTC")]
what2 <- what %>% fill(ac_effort, .direction = "down") %>% drop_na(ac_effort)

what3 <- filter(what2, straight == 'TRUE' & ac_effort == 'on')
