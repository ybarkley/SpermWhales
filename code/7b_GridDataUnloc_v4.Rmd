---
title: "Add unlocalized whales to Data"
...
This code is for adding in unlocalized encounters ad hoc to the 10 km localized data set. YB found that the env data for the localized and trackline locations of the localized whales were the same, but did not include the unlocalized encounters in the original gridding and effort calculations. Therefore, after deciding to include them, YB added them into the existing centroid data for 10km scale afterwards.
This was done by essentially finding the matching centroid to the trackline location and changing the pa value from 0 to 1, and then matching that up with the envData for the same centroid.
FYI: effCells and gridIndex are the columns for linking things up.
...
pmDets <- rename(pmDets, Longitude =lon2, # lon2, #
                         Latitude=lat, #lat, #
                         distance = pdist)
...
pmDetsub <- dplyr::filter(pmDets, survey == surveynum) # all sperm whales
...
saveRDS(gridEffort, here::here(paste0('output/gridEffort/', gridsize, ' km-', loctype), paste0('gridEffort', surveynum, '_', gridsize, 'km_', loctype2, '_unloc_raw.rda')))

saveRDS(centroidAll, here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids', surveynum, '_', gridsize, 'km_', loctype2, '_unloc_raw.rda')))
```
...
#If needed, just remove the detection
...
GridEff <- data.frame(gridEffort$effort$area[GridSurround$values[1:3]]) # get effort in surr cells
...
CentSurround <- filter(centroidUnloc, effCells %in% GridSurrDF$gridID) #these cells all have effort
...
```{r}
saveRDS(gridEffort, here::here(paste0('output/gridEffort/', gridsize, ' km-', loctype), paste0('gridEffort', surveynum, '_', gridsize, 'km_', loctype2, '_unloc.rda')))
```

#5. Run centroids again
```{r}
centroidUnloc <- gridCentroid(gridEffort, gpsAll)
centroidUnloc <- centroidAll
```
...
(all_prz <- dplyr::filter(centroidUnloc, pa>0))  
...
if((nrow(all_prz) == nrow(pmDetsub))==TRUE){
  ...
  centroidAlltmp <- filter(centroidUnloc, pa != 1)     #remove the rows with presences
...
centroidUnloc <- rbind(centroidAlltmp, all_prz)       #Should equal the same rows as before since just changing the value in pa
...
dbl_prz <- dplyr::filter(centroidUnloc, abs(diff_mins) > 1440 & pa > 0)
# dbl_prz <- dplyr::filter(unlocPres, abs(diff_mins) > 1440 & pa > 0)
...
dbl_abz <- filter(centroidUnloc, abs(diff_mins) > 3200 & pa == 0) #double effort within 2-3 days? Threshold based on grid data
...
sgl_abz <- dplyr::filter(centroidUnloc, abs(diff_mins) <= 3200 & pa == 0) # all times are within 2-3 days, which is fine bc datasets are monthly
SGL_abz <- dplyr::select(sgl_abz, `UTC` = start, everything(), -end)

sgl_prz <- dplyr::filter(centroidUnloc, abs(diff_mins) <= 1440 & pa > 0)
...
if (nrow(pmDetsub) == nrow(dplyr::filter(ALLcentroids, pa == 1))){
 ...
 saveRDS(ALLcentroids, here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids', surveynum, '_', gridsize, 'km_', loctype2, '_unloc.rda')))
  print('Shit is saved')
} else {
...
#Filter unlocalized whales
From centroids, get the unlocalized whales into their own dataframe
```{r}
centroidLoc <- readRDS(here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids',  surveynum, '_', gridsize, 'km_', loctype2, '.rda')))
ALLcentroids <- readRDS(here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids',  surveynum, '_', gridsize, 'km_', loctype2, '_unloc.rda')))
CompleteStart <- readRDS(here::here(paste0('output/envData/', gridsize, ' km-', loctype), paste0('CompletePm_', gridsize, 'km_', loctype2, '_scaledOLD.rda')))
gridEffort <- readRDS(here::here(paste0('output/gridEffort/', gridsize, ' km-', loctype), paste0('gridEffort', surveynum, '_', gridsize, 'km_', loctype2, '_unloc.rda')))
#sort by survey
CompleteLoc <- filter(CompleteStart, survey == surveynum)
CompleteLocAnti <- filter(CompleteStart, survey != surveynum)

centroidLoc <- ALLcentroids

pmUnlocGrid <- filter(gridEffort$detections, loc == 0 & sid == 999) #subset for unlocalized whales 

# filter(pmUnlocGrid, gridIndex %in% ALLcentroids$effCells)
unlocPres <- filter(centroidLoc, effCells %in% pmUnlocGrid$gridIndex) #filter the pull out the cells in AllCentroids that match the gridIndex

####change centroids####
unlocOrig <- filter(centroidLoc, effCells %in% unlocPres$effCells) #from original centroid data for the survey, pull out the cells with unloc whales
unlocOrig<-arrange(unlocOrig, UTC) # sort by UTC
unlocPres <- arrange(unlocPres, UTC)
unlocPres2 <- data.frame(unlocOrig, unlocPres$UTC,unlocPres$effCells, unlocPres$pa )
unlocPres3 <- select(unlocPres2, 1:6, 11, 8) #reorder to keep columns with pa from the unloc whales
colnames(unlocPres3)[7] = 'pa'
centLoc <- filter(centroidLoc, !(effCells %in% unlocPres$effCells)) 
centroidLoc2 <- bind_rows(centLoc, unlocPres3)
saveRDS(centroidLoc2,here::here(paste0('output/centroids/', gridsize, ' km-', loctype), paste0('centroids',  surveynum, '_', gridsize, 'km_', loctype2, 'NEW.rda')))



####change completed env data####
unlocOrig <- filter(CompleteLoc, effCells %in% pmUnlocGrid$gridIndex) #from original centroid data for the survey, pull out the cells with unloc whales
unlocOrig<-arrange(unlocOrig, UTC) # sort by UTC
#save row for 1705
A257_1705 <- subset(unlocOrig, acid ==257)
#1604/1705/1706
unLocAcDat <- filter(pmUnlocGrid, gridIndex %in% unlocOrig$effCells)
colnames(unLocAcDat)[43] <- 'effCells'
unLocAcDat <- unLocAcDat[, -c(27:42)]
unLocAcDat <- arrange(unLocAcDat, 'UTC')

merged <-merge(unlocOrig[, c(1,2,8,9,11,12,18:29,44)], unLocAcDat, by='effCells' )
merged2<-merged %>% rename(UTC=UTC.x, UTCp = UTC.y, Longitude=Longitude.x, Latitude = Latitude.x, Latitudep = Latitude.y, Longitudep = Longitude.y)
merged2$pa <- 1
# merged2$pa[6]<-2 #1705 DON'T DO THIS

combDat <- merged2[, c("pa","UTC","UTCp","ID","survey","acid","sid","effCells","EffArea","actualArea", "Longitude",  "Latitude","Longitudep", "Latitudep",  "lon_trk",    "lat_trk",    "distance",   "sst","chla"   ,    "temp600" ,   "wavepow"   , "bath_m"   ,  "slope" ,     "aspect", "dist" ,  "ssh", "sshsd","eke","d2smt" ,     "peak" ,      "itrk" ,      "type" ,      "nclk" ,      "minutes" ,   "seconds",    "cpm", "cps" ,"loc", "grpsize","nmin","rating" ,"click_code", "array",   "year")]      

#1641
unlocPres <- arrange(unlocPres, UTC)
unlocPres2 <- data.frame(unlocOrig, unlocPres$UTC, unlocPres$pa )
unlocPres3 <- select(unlocPres2, 46, 2:44) #reorder to keep columns with pa from the unloc whales
colnames(unlocPres3)[1] = 'pa'

zerosPa <- filter(unlocPres3, pa ==0)  #take out any with pa=0, save for later

onesPa <- filter(unlocPres3, pa !=0) #same as unlocOrig
unLocAcDat <- filter(pmUnlocGrid, gridIndex %in% onesPa$effCells)
colnames(unLocAcDat)[43] <- 'effCells'
unLocAcDat <- unLocAcDat[, -c(27:42)]
unLocAcDat <- arrange(unLocAcDat, 'UTC')

merged <-merge(onesPa[, c(1,2,8,9,11,12,16,18:29,44)], unLocAcDat, by='effCells' )
merged2<-merged %>% rename(UTC=UTC.x, UTCp = UTC.y, Longitude=Longitude.x, Latitude = Latitude.x, Latitudep = Latitude.y, Longitudep = Longitude.y)
merged2$pa <- 1
...
envDatLoc2 <- filter(CompleteLoc, !(effCells %in% unlocPres$effCells)) 
CompleteLoc2 <- bind_rows(envDatLoc2, combDat, A257_1705)
# CompleteLoc2 <- bind_rows(envDatLoc2, unlocOrig)

CompleteEnd <- bind_rows(CompleteLocAnti, CompleteLoc2)#, zerosPa)
nrow(CompleteStart) == nrow(CompleteEnd)

# #reconcile 1705 to put in A257
# CompleteNEW <- readRDS(here::here(paste0('output/envData/', gridsize, ' km-', loctype), paste0('CompletePm_', gridsize, 'km_', loctype2, '_scaledNEW.rda')))
# 
# CompleteNEW1705pa1 <- filter(CompleteNEW, survey==1705 & pa >0)
# 
# changethis <-CompleteNEW1705pa1[which(CompleteNEW1705pa1$acid ==256), ] 
# changethis$pa <-1 
# 
# CompleteNEW2 <- filter(CompleteNEW, !(effCells %in% c(changethis$effCells))) #remove it from original NEW version
# 
# CompleteNEW3 <- bind_rows(CompleteNEW2, changethis,A257_1705 )

saveRDS(CompleteNEW3,here::here(paste0('output/envData/', gridsize, ' km-', loctype), paste0('CompletePm_', gridsize, 'km_', loctype2, '_scaledNEW.rda')))
...
pdf(here::here('figures', paste0('Grid', surveynum, '_', gridsize, 'km_', loctype, '_UnlocCHECK.pdf')), width=11,height=8)
