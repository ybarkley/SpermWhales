...
SwEnvData_mapb <- readRDS(here::here( paste0('output/models/', loctype, '/data/', 'CompletePm_', gridsize, 'km_', loctype2, '_EDIT.rda') ))
...
#Nov 29
gps1641 <- gridEffort$gps
gps1641$year <- 2010
gps1303 <- gridEffort$gps
gps1303$year <- 2013
gps1604 <- gridEffort$gps
gps1604$year <- 2016
gps1705 <- gridEffort$gps
gps1705$year <- 2017
gps1706 <- gridEffort$gps
gps1706$year <- 2017
whalePts <- subset(SwEnvData_mapb, pa >0)
```

```{r}
#"#d64161", "#ff7b25",  "#86af49", "#6b5b95"
ggplot(gps1641, aes(Longitude, Latitude)) +
    geom_point(color='gray66', size=0.5, pch=16) +
  geom_point(gps1303, mapping=aes(Longitude, Latitude),color='gray66',size=0.5, pch=16) +
  geom_point(gps1604, mapping=aes(Longitude, Latitude),color='gray66',size=0.5, pch=16) +
  geom_point(gps1705, mapping=aes(Longitude, Latitude),color='gray66',size=0.5, pch=16) +
  geom_point(gps1706, mapping=aes(Longitude, Latitude),color='gray66',size=0.5, pch=16) +   theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
geom_point(SwEnvData_p, mapping=aes(Longitude, Latitude, color=factor(year)), size=1.5) + 
  scale_color_manual(values = c("#0040ff", "#ff7b25",  "#00e6e6", "#ff1ac6"), name='Year') + 
  scale_y_continuous(name='Latitude',breaks=seq(0,30,2)) +
  scale_x_continuous(name='Longitude') +
  geom_polygon(data = hawaiiMap, aes(x = Longitude2, y = Latitude, group = group), fill = '#86af49', color = 'black') +
  coord_fixed(1) +
  ggtitle("All Sperm Whale Encounters") + 
      scalebar(x.min = 180, x.max = 185,
                   y.min = 16, y.max = 20, dist = 250, dist_unit = "km",
             transform = TRUE, model = "WGS84")
  
ggsave(paste0(dir, '/SW_TracklineMap.png'), width=8, height = 6, units='in', dpi=300)

```


#Map of all sperm whales
```{r}
ggplot(SwEnvData_mapb, aes(Longitude, Latitude)) +
  ...
  scale_x_continuous(name='Longitude') +  #, breaks=round(Longitude, 1) ) +
   scale_shape_manual(values=c(8, 17, 15, 1),name='Enc Type', labels=c("Ac_Localized", "Ac_Trackline","Sighting")) +
 scale_color_manual(values = c("#d64161", "#ff7b25",  "#86af49", "#6b5b95"), name='Year') + #feb236", "#92a8d1",
...
## For Jeff
Oct 12, 2020- depths of constant temp, don't have, but have depths? - NOT WHAT HE WANTED. WANTED DEPTH OF WATER COLUMN WHERE IT WAS 6DEG C
...
ggplot(SwEnvData_mapb, aes(Longitude, Latitude)) +
  geom_point(aes(color=depth.r, shape=factor(year)), size=1.2) +
  geom_point(PmNonForPres, mapping=aes(Longitude, Latitude, fill='Whales'), pch=10, color='darkmagenta')  +
  ...
  # scale_x_continuous(name='Longitude', breaks=round(long,1)) +
   scale_shape_manual(values=c(19, 3, 8, 18),
                     name='Survey Year')   +
 scale_color_gradientn(colors = c("#33cc33","#99ff66","#ff6666","#ff0000"), name="Depth (m)" ) +
  scale_fill_manual(values = 10, name='') +
  geom_polygon(data = hawaiiMap, aes(x = Longitude2, y = Latitude, group = group), fill = 'black', color = 'black') +
  coord_fixed(1) +
  ggtitle("Depth") +
...
## Encounter Rate Foraging ####
PmFor <- readRDS(here::here( paste0('data/Pm_ForagingOnly.rda') ))
PmForAbs <- subset(PmFor, pa == 0)
PmForPres <- subset(PmFor, pa > 0)
ggplot(PmFor, aes(Longitude, Latitude)) +
  geom_point(PmForAbs, mapping = aes(color=pa, shape=factor(year)), size=1.2) +
  geom_point(PmForPres, mapping = aes(color=pa, shape=factor(year)), size=1.2) +
  # geom_point(SwEnvData_p, mapping=aes(Longitude, Latitude, fill='Whales'), pch=10, color='darkmagenta')  +
  # scale_color_discrete(name = "Encounter Rate") +  #if color is factor
  scale_shape_manual(values=c(19, 3, 8, 18),
                     name='Survey Year')  +
  theme_bw()  +
    # scale_y_continuous(name='Latitude',breaks=seq(0,30,2))+
  # scale_x_continuous(name='Longitude', breaks=round(long,1)) +
   # scale_shape_manual(values=c(19, 3, 8, 18),
                     # name='Survey Year')   +
  # scale_fill_manual(values  =c("#AFEEEE", '#00FA9A', '#FF8C00', '#8B0000'), name="Encounter Rate") +
 scale_color_gradientn(colors =c("#AFEEEE", '#FF8C00', '#FF00FF', '#8B0000'), name=expression(paste("Number of Groups per cell")))  +
  # scale_fill_manual(values = 10, name='') +
  geom_polygon(data = hawaiiMap, aes(x = Longitude2, y = Latitude, group = group), fill = 'black', color = 'black') +
  coord_fixed(1) +
  ggtitle("Foraging Encounter Rate by Year") +
  theme(legend.position="right", legend.box='vertical',legend.text = element_text(size = 10),legend.title = element_text(size=9),plot.title = element_text(hjust = 0.5))+
  guides(shape = guide_legend(nrow = 2, byrow = TRUE))

ggsave(paste0(dir, 'SwMap_EncRate3.png'), width=8, height = 6, units='in', dpi=300)


## Encounter Rate Non-Foraging ####
PmNonFor <- readRDS(here::here( paste0('data/Pm_NonForagingOnly.rda') ))
PmNonForAbs <- subset(PmNonFor, pa == 0)
PmNonForPres <- subset(PmNonFor, pa > 0)
nrow(PmNonForAbs)
ggplot(PmNonFor, aes(Longitude, Latitude)) +
  geom_point(PmNonForAbs, mapping = aes(color=pa, shape=factor(year)), size=1.2) +
  geom_point(PmNonForPres, mapping = aes(color=pa, shape=factor(year)), size=1.2) +
  # geom_point(SwEnvData_p, mapping=aes(Longitude, Latitude, fill='Whales'), pch=10, color='darkmagenta')  +
  # scale_color_discrete(name = "Encounter Rate") +  #if color is factor
  scale_shape_manual(values=c(19, 3, 8, 18),
                     name='Survey Year')  +
 ...
 scale_color_gradientn(colors =c("#AFEEEE", '#FF8C00', '#FF00FF', '#8B0000'), name=expression(paste("Number of Groups per cell")))  +
  ...
  geom_polygon(data = hawaiiMap, aes(x = Longitude2, y = Latitude, group = group), fill = 'black', color = 'black') +
  coord_fixed(1) +
  ggtitle("Non-Foraging Encounter Rate by Year") +
  theme(legend.position="right", legend.box='vertical',legend.text = element_text(size = 10),legend.title = element_text(size=9),plot.title = element_text(hjust = 0.5))+
  guides(shape = guide_legend(nrow = 2, byrow = TRUE))

ggsave(paste0(dir, 'SwMap_NonForEncRate.png'), width=8, height = 6, units='in', dpi=300)
...
SwEnvData_mapb4 <- filter(SwEnvData_mapb, cpm != 'Inf')
...
ggplot(SwEnvData_mapb, aes(cpm, grpsize)) +
