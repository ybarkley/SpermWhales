...
sw <- read_csv(here::here('output', 'SpermiesFinal_20200525.csv'))
...
SwEnvDataf <- filter(SwEnvData, itrk == 1)

write.csv(SwEnvData, here::here('output', 'SpermiesWithEnvData_20200527.csv'), row.names = F)




write.csv(SwEnvDataLocTrk, here::here('output', 'SpermiesWithEnvDataLocTrk_20200527.csv'), row.names = F)
```
...
#Left vs Right for Localized Data using "Best" location
```{r}

#filter for localized data and predictors only
SwEnvDataLocA <- filter(SwEnvData_scale, SwEnvData_scale$loc ==1 & SwEnvData_scale$peak == 'A') #peak A
SwEnvDataLocB <- filter(SwEnvData_scale, loc ==1 & peak == 'B') #peak B
SwEnvDataLocT <- filter(SwEnvData_scale, loc ==1) #trackline
...
par(mfrow=c(3, 2))
hist(SwEnvData_pred$sstAQ_m, main = expression(paste("SST ("^"o", "C",")")))
hist(SwEnvData_pred$temp600C, main = 'Temp @ 600m')
hist(SwEnvData_pred$chla_m, main = 'Chlorophyll a')
hist(SwEnvData_pred$par_m, main = expression(paste("PAR (E m"^"-2", "day"^"-1",")")))
hist(SwEnvData_pred$wind_ms, main = 'Wind Speed (m/s)')

hist(SwEnvData_pred$bath, main="Bathymetric Depth (m)")
hist(SwEnvData_pred$slp_deg,  main = 'Slope (deg)')
hist(SwEnvData_pred$asp_deg,  main = 'Aspect (deg)')
hist(SwEnvData_pred$d2land_km, main = 'Dist to Land (km)')
hist(SwEnvData_pred$d2smt_km, main = 'Dist to Seamount (km)')


par(mfrow=c(2, 2))
hist(SwEnvData_pred$wavepow, main = 'Wave Power (kW/m)')
hist(SwEnvData_pred$ssh, main = 'SSH')
hist(SwEnvData_pred$sshsd, main = 'SSHsd')
hist(SwEnvData_pred$eke, main = 'EKE')
