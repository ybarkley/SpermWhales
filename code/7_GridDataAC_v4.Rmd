...
```{r}
#load grid data
gridEffort <- readRDS(here::here( paste0('output/gridEffort/AcOnly/', gridsize, ' km-', loctype, '/gridEffort', surveynum, '_', gridsize, 'km_', loctype2, '_AC.rda') ))
...
centroidAll <- readRDS(here::here( paste0('output/centroids/', gridsize, ' km-', loctype, '/centroids',  surveynum, '_', gridsize, 'km_', loctype2, '.rda') ))
#ac only
centroidAll <- readRDS(here::here( paste0('output/centroids/', gridsize, ' km-AcOnly', '/centroids',  surveynum, '_', gridsize, 'km_', loctype2, '_AC.rda') ))

CompletePm25 <-readRDS(here::here( paste0('output/envData/', gridsize, ' km-', loctype, '/CompletePm_',gridsize, 'km_', loctype2, '_scaled.rda') ))
CompletePm25sub <- filter(CompletePm25, survey==surveynum)
...
### 9/21/20 ACOUSTICS ON ONLY for Acoustics Only Models
...
#when vis is ON and acoustics is OFF, effort should be FALSE. Only want time when ac is on
effVischk=gpsAll[which(gpsAll$join.veffort=='on' & gpsAll$straight==TRUE),]
effVischkON = effVischk[which(effVischk$join.aeffort=='off'),]  #For ON for acoustics only, should have FALSE in effort column
which(effVischkON$effort == TRUE) #should be zero

#when acoustics is ON and vis is OFF, effort should still be TRUE
effACchk=gpsAll[which(gpsAll$join.aeffort=='on'& gpsAll$straight==TRUE),]
...
pmDetsub <- dplyr::filter(pmDets, survey == surveynum,  loc ==1 | loc == 0 & sid < 999 & acid <999 ) # all sperm whales
...
saveRDS(gridEffort, here::here( paste0('output/gridEffort/AcOnly/', gridsize, ' km-', loctype, '/gridEffort', surveynum, '_', gridsize, 'km_', loctype2, '_ACraw.rda') ))

saveRDS(centroidAll, here::here( paste0('output/centroids/AcOnly/', gridsize, ' km-', loctype, '/centroids', surveynum, '_', gridsize, 'km_', loctype2, '_ACraw.rda') ))
```
...
pdf(here::here('figures', paste0('Grid', surveynum, '_', gridsize, 'km_', loctype, '_CHECKeffAC.pdf')), width=11,height=8)
...
points(x=effchg$Longitude, y=effchg$Latitude	, col='orange', pch = 19, cex=0.4, lwd = 0.07 )
...
```{r}
saveRDS(gridEffort, here::here(paste0('output/gridEffort/', gridsize, ' km-', loctype), paste0('gridEffort', surveynum, '_', gridsize, 'km_', loctype2, 'AC.rda')))
```
...
```{r}
#Change variables based on the folders as needed.
CombCents = NULL
# AcOnlyEnv = NULL
# pa1Check = NULL
