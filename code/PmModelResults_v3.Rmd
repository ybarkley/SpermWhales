...
Previous models did not incorporate a temporal component, such as year or month, which is necessary given the time span of the data set over 4 years. Each model was revised to include variables for year and yearly quarter to account for temporal variation within the data.

* Results for the new Acoustics-Only models showed no changes to the important variables or their relationship with sperm whale encounter rate, suggesting there were no temporal effects. The conclusions remain the same.  
...
* Results for the new Foraging Models:   
  + Foraging Model 1 included yearly quarter as a significant variable. The effects were similar to Combined Model 1, with a peak at the third quarter.
  + Foraging Model 2 showed no change when including the temporal variables.
 
* Results for the new Social Group Models:   
  + Social Group Model 1 included year as a significant variable, showing a decline in encounter rate of social groups from 2010-2017. 
  + Social Group Model 2 with the spatial smoother and temporal variables resulted in a non-significant spatial smoother as before. The overall model ended up being the exact same as Social Group Model 1. 
  Very similar conclusions can be drawn for the Social Group models. The negative relationship with year is somewhat interesting, but it's unclear whether that means there were truly less social groups or if it was random chance that they weren't detected in the 2016 and 2017 surveys.
...
#year
PmAcOnly$year <- 2010
PmAcOnly$year[PmAcOnly$survey==1303] <- 2013 
PmAcOnly$year[PmAcOnly$survey==1604] <- 2016
PmAcOnly$year[PmAcOnly$survey==1705] <- 2017
PmAcOnly$year[PmAcOnly$survey==1706] <- 2017
PmAcOnly2 <- select(PmAcOnly, last_col(), everything())

#quarter: test on PmAcOnly2, then add to PmAcOnly
dateSplit = data.frame(1, time=as.character(as.Date(PmAcOnly$UTC)))  #some data finagling
dateSplit2 = separate(dateSplit, "time",  c("year", "month", "day")) #splits date into year, month, day
PmAcOnly2$month = as.numeric(dateSplit2$month) #add to PmAcOnly2 to test
PmAcOnly2$qtr = 1
PmAcOnly2$qtr[PmAcOnly2$month >= 4 & PmAcOnly2$month <=6] <- 2
PmAcOnly2$qtr[PmAcOnly2$month >= 7 & PmAcOnly2$month <=9] <- 3
PmAcOnly2$qtr[PmAcOnly2$month >= 10 & PmAcOnly2$month <=12] <- 4
PmAcOnly2 <- select(PmAcOnly2, last_col(), everything())

PmAcOnly$qtr <- PmAcOnly2$qtr

# saveRDS(PmAcOnly, here::here(  paste0('output/models/', loctype, '/data/', 'CompletePm_', gridsize, 'km_', loctype2, '_scaled.rda') ))
...
saveRDS(trainAcOnly, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_S999.rda')  ))
saveRDS(testAcOnly, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_S999.rda')  ))
...
#quarter: test on PmCombo2, then add to PmCombo
dateSplit = data.frame(1, time=as.character(as.Date(PmCombo$UTC)))  #some data finagling
dateSplit2 = separate(dateSplit, "time",  c("year", "month", "day")) #splits date into year, month, day
PmCombo2$month = as.numeric(dateSplit2$month) #add to PmCombo2 to test
PmCombo2$qtr = 1
PmCombo2$qtr[PmCombo2$month >= 4 & PmCombo2$month <=6] <- 2 
PmCombo2$qtr[PmCombo2$month >= 7 & PmCombo2$month <=9] <- 3
PmCombo2$qtr[PmCombo2$month >= 10 & PmCombo2$month <=12] <- 4
PmCombo2 <- select(PmCombo2, last_col(), everything())
...
saveRDS(trainComb, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_Comb2.rda')  ))
saveRDS(testComb, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_Comb2.rda')  ))
...
trainComb <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Train_',   gridsize, 'km_', loctype2, '_Comb2.rda')  ))
testComb <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Test_',     gridsize, 'km_', loctype2, '_Comb2.rda')  ))
...
**UPDATE:**  
**No change in model results with the inclusion of temporal variables.**  
...
gam(EncRate ~ s(Depth, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(log.EKE, k=3) + offset(log.effort)
...
modAcOnly <- gam(pa ~ s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(sst.r, k=3) + s(log.chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(sshsd, k=3) + s(log.EKE, k=3) + s(log.wp, k=3) + offset(log.effort) + year + qtr , data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly)

modAcOnly2 <- gam(pa ~ s(Depth, k=3) + s(sst.r, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(log.EKE, k=3) + offset(log.effort)  , data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly2)

modAcOnly3 <- gam(pa ~ s(Depth, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(log.EKE, k=3) + offset(log.effort)  , data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly3)

#CONCLUSION: Temporal variables were not significant. No temporal patterns in data.
```



```{r echo=FALSE, fig.cap = 'Acoustics-Only Model 1: The partial effects plots show the component effect of each simple smooth term without a spatial smoother.', fig.width=6, fig.height=4}
...
scheme = 1, xlab = expression("log.EKE (m"^2* "/s"^2*")"), shade = T, shade.col = 'honeydew2', seWithMean = TRUE)
...
gam(EncRate ~ s(Longitude, Latitude, k=10) + s(SSH, k=3)  + s(SSHsd, k=3) + offset(log.effort)
...
#Checking if temporal variation exists by including year and quarter into the models
#Add quarter and year to full model first
modAcOnly2D <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(sst.r, k=3) + s(log.chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(sshsd.r, k=3) + s(log.EKE, k=3) + s(log.wp, k=3) + offset(log.effort) + year + qtr , data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly2D)

modAcOnly2D2 <- gam(pa ~ s(Longitude, Latitude, k=10) + s(sst.r, k=3)  + s(SSH, k=3) +  s(sshsd.r, k=3) + offset(log.effort)  , data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly2D2)

modAcOnly2D3 <- gam(pa ~ s(Longitude, Latitude, k=10) + s(SSH, k=3) +  s(sshsd.r, k=3) + offset(log.effort)  , data = trainS999, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modAcOnly2D3)

#CONCLUSION: Temporal variables were not significant. No temporal patterns in data. Same results as before.
```
...
```{r echo=FALSE, eval=FALSE, fig.cap='Temperatures at a depth of 584 m for September 2010.', fig.width=5, fig.height=3}
...
```{r echo=FALSE, eval=FALSE, fig.cap='Temperatures at a depth of 584 m across survey years. Whales represent acoustic encounters only.', fig.width=5, fig.height=3}
## image ####
require(here)
...
The negative relationship between SSH and sperm whale encounter rate indicated lower encounter rates in areas with higher SSH. The standard deviation in SSH showed higher encounter rates at 0 m and 0.025 m but lower encounter rates as SSHsd approached an approximate median value of 0.012 m. 
Keeping in mind the 8 km spatial resolution of the monthly averaged SSH and SSHsd variables, higher values suggest areas of upwelling and potentially higher productivity, with lower values indicating areas of downwelling that could be associated with both higher and lower productivity. 
Results show a clear negative relationship between SSH and sperm whale encounter rate for both models. However, that doesn't necessarily mean they are typically found in unproductive waters since lower SSH could represent productive areas associated with downwelling. 
The SSHsd ranges between ~0-2.5 cm, with the highest predictions of sperm whale encounters occurring at both ends of this range. This indicates that there is relatively low variation in the surrounding SSH (within an 8 km neighborhood) of the whale encounters. 
The stability in SSH may represent areas that are less dynamic. Given model relationships, it's difficult to interpret the biological relevance of these variables for sperm whale encounters especially with low gradients within the data itself. 
...
**UPDATED:** 
After including year and quarter to account for temporal variability in the data, the best Combined model selected 'quarter' as a significant variable, and removed SSH compared to previous results. All other significant variables remained the same and showed similar relationships.

gam(EncRate ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(Qtr, k=3) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")


**OLD**
*Results from the Combined model found SST, logChla, temp at 584 m, SSH, and SSHsd to be statistically significant.*  

*gam(EncRate ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3)  + offset(log.effort), data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")*
...
modComboBest <- gam(pa ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(qtr, k=3) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
...
```{r echo=FALSE, fig.cap= 'Combined Model 1: Partial effects plots without a spatial smoother.', fig.width=6, fig.height=5}
...
plot(modComboBest, select=2, residuals = FALSE, pch = 20, shade = T,  ylim = c(-6,2),
scheme = 1, shade.col = 'lightcyan', all.terms = TRUE, xlab='log Chl a (mg m-3)' , seWithMean = TRUE)
...
modCombo <- gam(pa ~ s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo) 

# quarter was significant and made a difference in chla if qtr was a factor or smoother!
modCombo2 <- gam(pa ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(qtr, k=3) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo2)


#CONCLUSION: Quarter temporal variables IS significant. The number of sperm whales does differ by quarter.
```
...
**UPDATED:** 
No time variables were selected for the model when including the spatial smoother. The model resulted in a similar pattern in the spatial smoother as well as similar patterns in the other signifcant variables. The only difference for the updated Combined Model 2 is that it included SST as a significant variable.

gam(EncRate ~ s(Longitude, Latitude, k=10) + s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
...
*gam(EncRate ~ s(Longitude, Latitude, k=10) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort), data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")*


```{r}
#look at some stuff in the training data
trainComb$pa2 <- ifelse(trainComb$pa > 0, 1, 0)
trainComb <- dplyr::select(trainComb, pa2, everything())
trainComb[which(trainComb$effCells==1746), ]
nrow(trainComb[which(trainComb$survey==1641 & trainComb$pa >0), ])

```

```{r echo = FALSE}
modComboBest2D <- gam(pa ~ s(Longitude, Latitude, k=10) + s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
# summary(modComboBest2D) #WAS modCombo2D3
  
  #gam(pa ~ s(Longitude, Latitude, k=10) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort), data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
...
modCombo2Db <- gam(pa2 ~ s(Longitude, Latitude, k=10) + s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + year + qtr , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo2Db)


### USE SMOOTHER FOR TIME VARIABLES TO BE CONSISTENT WITH ABOVE 
modCombo2D2 <- gam(pa ~ s(Longitude, Latitude, k=10) + s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo2D2)

# TIME was NOT significant!
modCombo2D3 <- gam(pa ~ s(Longitude, Latitude, k=10) + s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) , data = trainComb, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modCombo2D3)
...
plot(modComboBest2D, select=2, residuals = FALSE, pch = 20, cex = 0.25, ylim = c(-2,2),
     scheme = 1, shade = T, shade.col = 'lightcyan', all.terms = TRUE, seWithMean = TRUE, xlab = 'SST (ºC)' )

plot(modComboBest2D, select=3, residuals = FALSE, pch = 20, shade = T, ylim = c(-6,2),
     scheme = 2, shade.col = 'lightcyan', all.terms = TRUE, xlab='logChla (mg m-3)', seWithMean = TRUE )

plot(modComboBest2D, select=4, residuals = FALSE, pch = 20, shade = T, ylim = c(-2,2),
     scheme = 2, shade.col = 'lightcyan', all.terms = TRUE, xlab='Temp @ 584 m (°C)', seWithMean = TRUE )

plot(modComboBest2D, select=5, residuals = FALSE, pch = 20, shade = T, ylim = c(-2,2),
     scheme = 2, shade.col = 'lightcyan', all.terms = TRUE, xlab='SSHsd (m)', seWithMean = TRUE )
...
PmFor1 <- filter(PmCombo, grepl('r|k', click_code))   # the 'k' implies ck for creaks, c also for codas
...
PmGroup1 <- filter(PmCombo, grepl('cd', click_code))
PmGroup0 <- filter(PmCombo, !grepl('cd', click_code))
...
saveRDS(trainForg, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_Huntb.rda')  ))
saveRDS(testForg, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_Huntb.rda')  ))
...
saveRDS(trainSoc, here::here(  paste0('output/models/',loctype, '/data/Train_', gridsize, 'km_', loctype2, '_Socialb.rda')  ))
saveRDS(testSoc, here::here(  paste0('output/models/',loctype, '/data/Test_', gridsize, 'km_', loctype2, '_Socialb.rda')  ))
...
```{r echo=FALSE}
## FORAGE/SOCIAL: LOAD TRAIN and TEST ####

#seed 1
trainHunt <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Train_',   gridsize, 'km_', loctype2, '_Huntb.rda')  ))
testHunt <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Test_',     gridsize, 'km_', loctype2, '_Huntb.rda')  ))
...
#seed 1
trainSoc <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Train_',   gridsize, 'km_', loctype2, '_Socialb.rda')  ))
testSoc <- readRDS(here::here(  paste0('output/models/',loctype, '/data/Test_',     gridsize, 'km_', loctype2, '_Socialb.rda')  ))
...
**UPDATED:**  
Although yearly quarter was significant, the same variables from the previous foraging model were also significant with similar relationships.

gam(pa ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + s(qtr, k=3)


```{r echo=FALSE, eval=FALSE}
## FORAGE 1: TIME VARIABLES ####
#Checking if temporal variation exists by including year and quarter into the models
#Add quarter and year to full model first
# FYI!!! including year as smooth makes a difference
modForg <- gam(pa ~ s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForg) 

# quarter was significant and made a difference in chla if qtr was a factor or smoother!
modForg2 <- gam(pa ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(qtr, k=3) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForg2)

#CONCLUSION Foraging: Quarter temporal variables IS significant. The number of sperm whales does differ by quarter.

modForg2D <- gam(pa ~  s(Longitude, Latitude, k=10) + s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForg2D) 

modForg2D2 <- gam(pa ~  s(Longitude, Latitude, k=10) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort), data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForg2D2) 
```


```{r echo=FALSE, fig.cap = "Foraging Model 1: Partial effects plots without a spatial smoother, accounting for temporal variation." , fig.width=6, fig.height=5}

modForageBest <- gam(pa ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + s(qtr, k=3) + offset(log.effort), data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")

# summary(modForageBest)

par(mar=c(4,4,3,3),mfrow = c(3,2))
# plot(modForageBest)
plot(modForageBest, select=1, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='SST (°C)', seWithMean = TRUE, ylim = c(-4,3))#, main='Forage Model 1' )
...
plot(modForageBest, select=3, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='Temp at 584m (°C)', seWithMean = TRUE, ylim = c(-4,3))# , main='Forage Model 1' )

plot(modForageBest, select=4, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='SSHsd (m)', seWithMean = TRUE, ylim = c(-4,3))# , main='Forage Model 1' )

plot(modForageBest, select=5, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='Quarter of Year', seWithMean = TRUE, ylim = c(-4,3))# , main='Forage Model 1' )
```
...
**UPDATED:**  
**No changes in Foraging Model 2 with spatial smoother after accounting for temporal variation.**

gam(EncRate ~  s(Longitude, Latitude, k=10) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) 


```{r echo=FALSE, fig.cap = "Foraging Model 2: Partial effects plots with a spatial smoother." , fig.width=6, fig.height=4}
...
```{r echo=FALSE, fig.cap = "Foraging Model 2: Contour plot of spatial smoother. Purple dots represent acoustically detected encounters. Black dots are all data points with effort included in the model.", fig.width=6, fig.height=4}
par(mfrow = c(1,1))
...
#### Social Group models included a subset of the sperm whale encounters used for the Combined Models (sighted with concurrent acoustics, and acoustics only) but only included encounters that contained that contained codas, which indicate social behavior between individuals that may include females, males, and juveniles.
```{r echo=FALSE, eval=FALSE}
## SOCIAL: TIME VARIABLES ####
#Checking if temporal variation exists by including year and quarter into the models
#Add quarter and year to full model first
# FYI!!! including year as smooth makes a difference
modSoc <- gam(pa ~ s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modSoc) 

# quarter was significant and made a difference in chla if qtr was a factor or smoother!
modSoc2 <- gam(pa ~ s(Depth, k=3) + s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(year, k=3) + offset(log.effort) , data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modSoc2)

#CONCLUSION Social 1: YEAR temporal variables IS significant. The number of sperm whales does differ by quarter.

modSoc2D <- gam(pa ~  s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3) , data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modSoc2D) 

modForg2D2 <- gam(pa ~  s(Longitude, Latitude, k=10) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) , data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForg2D2) 
```
...
**UPDATED:**  
Adding in temporal component caused 'Year' to be significant. Depth and SSHsd were also selected compared to the previous version of this model.

gam(EncRate ~ s(Depth, k=3) + s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(year, k=3)


**OLD:**  
*Social Group Model 1 without time component*  

*gam(pa ~ s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + offset(log.effort), data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")*

```{r echo=FALSE, fig.cap= "Social Group Model 1: Partial effects plots, no spatial smoother.", fig.width=6, fig.height=5}
modSocBest <- gam(pa ~ s(Depth, k=3) + s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSHsd, k=3) + offset(log.effort) + s(year, k=3) , data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")
# summary(modSocBest)

par(mar=c(4,4,1,3),mfrow = c(3,2))
plot(modSocBest, select=1, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'goldenrod1', all.terms = TRUE, xlab='Depth (m)', seWithMean = TRUE, ylim = c(-5,4) )

plot(modSocBest, select=2, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'goldenrod1', all.terms = TRUE, xlab='SST (°C)', seWithMean = TRUE, ylim = c(-5,4) )

plot(modSocBest, select=3, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'goldenrod1', all.terms = TRUE, xlab='log Chl a (mg m-3)' , seWithMean = TRUE)

plot(modSocBest, select=4, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'goldenrod1', all.terms = TRUE, xlab='Temp at 584 m (°C)', seWithMean = TRUE, ylim = c(-5,4) )

plot(modSocBest, select=5, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'goldenrod1', all.terms = TRUE, xlab='SSHsd (m)', seWithMean = TRUE, ylim = c(-5,4) )

plot(modSocBest, select=6, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'goldenrod1', all.terms = TRUE, xlab='Year', seWithMean = TRUE, ylim = c(-5,4) )
```
...
* The spatial 2D smoother was not significant and results were the same as Social Group Model 1.


```{r echo=FALSE}
#2D smoother not signif, but save for plots
modSocBad2D <- gam(pa ~ s(Longitude, Latitude, k=10) + s(Depth, k=3) + s(dist, k=3) + s(d2smt, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(ssh, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + s(qtr, k=3), data = trainSoc, family = nb, link = 'log', select = TRUE, method = "REML")
# summary(modSocBad2d)
```



```{r echo=FALSE, eval=FALSE, fig.cap= "Social Group Model 2: Partial effects plots with spatial smoother (not significant).", fig.width=6, fig.height=5}
...
#### For social group, with spatial smoother ###
# #pulling the prediction and residual data from the model
...
table[3,] <- c("Social", "Model 1 (& 2)", paste0(nbExpDev, '%'), round(nbAIC, 2), round(nbMSEtrain,3), round(nbMSEtest,3))
...
The spatial contours for Foraging Model 2 predicted more foraging groups in a specific zone situated north of Laysan and Lisianski and east of Pearl and Hermes Reef. Upon closer inspection of that region, a line of seamounts exists northeast of these islands (Figure 16). The Distance to Seamounts variable was included in the models to relate potential areas of increased primary and secondary productivity caused by seamounts to their proximity with sperm whales, assuming that these more productive areas are tied to the potential increase in sperm whale prey. The bathymetry in this zone is relatively flat except for the areas closer to the atolls and the seamounts. The seamounts may be causing areas of local upwelling that influence sperm whale prey biomass. Figure 16 also shows a line of seamounts to the northeast outside the study area (Hawaii EEZ boundary). It would be interesting to survey those areas to evaluate the presence of foraging sperm whales, and other pelagic cetaceans, to evaluate its importance as foraging habitat.

Additionally, the same area where more foraging whales were present in the NWHI is also close to the Subtropical Frontal Zone (STFZ). The physical environment of the STFZ is highly dynamic, enhancing the chlorophyll concentration and zooplankton communities in this area. Higher trophic organisms (swordfish, tuna, loggerhead turtles) also frequent the area within the STFZ. Therefore, it is likely that pelagic species of cetaceans may benefit from the increase in biological productivity, including the foraging sperm whales at the most northern edge of the study area. 
...
```{r echo=FALSE, fig.cap='Map of the northwest Hawaiian Islands overlaid with seamounts (colored blobs). The red box includes the zone where more foraging groups are predicted to occur.', fig.width=6, fig.height=4, fig.align='center'}
## image ####
require(here)
...
Models examined sperm whale distribution patterns by relating the number of sperm whale encounters per 25-$km^2$ units to static and dynamic environmental variables available for this study area. The explained deviance of models was relatively low, but the relative prediction accuracy was consistent between the trained model and the predictions of the test data given the MSE values. Models that included a 2D smoother resulted in lower values of AIC. Most models consistently showed SSHsd, Chl-a, Temperature at 584m, and the spatial 2D smoother to predict patterns in sperm whale occurrence across all model types. Model results indicate that sperm whales are not necessarily found in deeper, more productive waters as hypothesized.

The Combined models contained all possible sperm whale encounters and showed similar results to models that only included acoustic encounters. This suggests that sighting data and acoustic encounters do not predict any obvious differences in distribution patterns. The behavior models provide insight into potential differences in habitat use for foraging groups and social groups, but further dedicated research is required to thoroughly address this theory.
