...
##For AcOnly 10 km and 25 km datasets with all surveys combined, had trouble downloading entire WP data at once. 
nc files were not saving.Subset total dataset by cruise like before.
```{r}
surveynum=1706

centroidSub <- subset(centroidData, survey == surveynum)
...
# matchedData <- matchEnvData(centroidData, nc=edinfoWP, fileName = here::here('output/envData', paste0(gridsize, ' km-', loctype, '/',surveynum, "_", gridsize, 'km_',loctype2, '_', envType, ".nc")))

#for subsets by survey
matchedData <- matchEnvData(centroidSub, nc=edinfoWP, fileName = here::here('output/envData', paste0(gridsize, ' km-', loctype, '/',surveynum, "_", gridsize, 'km_',loctype2, '_', envType, ".nc")))
...
envType = 'ssh'
matchedData <- matchEnvData(centroidData, nc=edinfoSSH)
matchedData2 <- matchEnvData(centroidData, nc=edinfoSSH, buffer(.04, .04, 0), FUN=c(mean))

#SD of SSH
matchedData3 <-matchEnvData(centroidData,nc=edinfoSSH, buffer=c(.04, .04, 0), FUN=sd)
