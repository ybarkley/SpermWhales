...
#check for duplicate detections in the same cells
...
GridEffArea <- gridEffort$effort$area[GridInd[,2]] # how much effort is in the cell? (Should be zero)
...
colnames(GridSurroundtmp) <- GridInd[,1] #include acid as column name
...
pdf(here::here('figures', paste0('Grid', surveynum, '_', gridsize, 'km_', loctype, 'noEff2.pdf')), width=11,height=8)
...
points(x=effchg$Longitude, y=effchg$Latitude	, col='orange', pch = 19, cex=0.07, lwd = 0.07 )
...
newCell <- GridCentJoin[c(1,4),1] # Select the gridIndex for the new cells based on the plot
...
```{r}
#Only run again if grid cells with no effort were replaced. Otherwise, no need.
centroidAll <- gridCentroid(gridEffort, gpsAll)
