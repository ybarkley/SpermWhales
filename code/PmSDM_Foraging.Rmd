...
modForage <- gam(pa ~  s(Longitude, Latitude, k=10) + s(depth.r, k=3) + s(distland.r, k=3)  + s(distseamt.r, k=3) +  s(SST, k=3) + s(log.Chla, k=3) + s(Temp584m, k=3) + s(SSH, k=3) + s(SSHsd, k=3) + s(log.eke, k=3) + s(log.wp, k=3) + offset(log.effort) + s(year, k=3) + qtr2 , data = trainHunt, family = nb, link = 'log', select = TRUE, method = "REML")
summary(modForage) 
...
# par(mar=c(4,4,3,2),mfrow = c(2,2))
# plot(modForageBest)
plot(modForageBest, select = 1, scheme = 2, lwd = 2) #select the first smoother, select = 1
...
plot(modForageBest, select=2, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='Temp at 584 m (°C)' , seWithMean = TRUE, ylim = c(-2,2))#, shift = coef(modForageBest)[1],	trans	= function(x)	exp(x))

plot(modForageBest, select=3, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab=expression("Chl-a (mg/m"^3*")"), seWithMean = TRUE, ylim = c(-8,2) )

plot(modForageBest, select=4, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='SSHsd (m)',seWithMean = TRUE, ylim = c(-2,2) )
...
plot(modNonForBest, select=2, residuals = FALSE, pch = 20, shade = T, scheme = 2, shade.col = 'lightsalmon', all.terms = TRUE, xlab='Depth (m)' , seWithMean = TRUE)


plot(modNonForBest, select = 1, scheme = 2, lwd = 2) #select the first smoother, select = 1
hawaiiMap <- readRDS(here::here(paste0('data/hawaiiMap.rda')))
hawaiiMap$Longitude2 = ifelse(hawaiiMap$Longitude<1, hawaiiMap$Longitude+360, hawaiiMap$Longitude)
